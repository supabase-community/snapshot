import { DataModel, DataModelModel, DataModelObjectField } from './dataModel.js'
import { pluralize, singularize, camelize, underscore } from 'inflection'
...
export type AliasModelNameConflict = {
  aliasName: string
...
export function computeAliases(
  ...
  merge(aliases, overrides)
  let conflicts = computeModelNameConflicts(dataModel, aliases)

  conflicts = attemptResolveModelNameConflicts(
    conflicts,
    aliases,
    dataModel,
    inflection
...
const computeModelNameConflicts = (
  dataModel: DataModel,
  aliases: Aliases
): AliasModelNameConflict[] => {
  const conflicts: AliasModelNameConflict[] = []
...
const attemptResolveModelNameConflicts = (
  ...
  const remainingConflicts: AliasModelNameConflict[] = []
    ...
    if (aliases[modelName]) {
      ...
      const aliasedModelValues = {
        ...
        uniqueConstraints: modelValues.uniqueConstraints.map((constraint) => {
          ...
          return {
            ...constraint,
            columns: constraint.columns.map((column) => fields[column]),
          }
        }),
      }
...
export function getAliasedDataModel(
  ...
  if (options?.inflection === undefined || options?.inflection === true) {
    ...
    options?.inflection !== undefined &&
    options?.inflection !== false
  ) {
    inflection = {
      ...standardInflection,
      ...options?.inflection,
