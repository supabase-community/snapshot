...
import { escapeKey } from '~/config/snapletConfig/v2/generateTypes/escapeKey.js'
import {
  Fingerprint,
  isJsonField,
  jsonSchemaToTypescriptType,
} from './dataModel/fingerprint.js'
...
export async function generateTypes(props: {
  ...
  const { dataModel, fingerprint, adapterName = 'pg' } = props
  return [
    generateHelpers(),
    ...
    generateClientAdapterTypes(adapterName),
  ].join(os.EOL)
}

export async function generateConfigTypes(props: {
  dataModel: DataModel
...
${dataModel.models[modelName].fields
  ...
  return [inflection, override, alias].join(os.EOL)
}
...
const generateClientTypesByAdapter = {
  pg() {
    return `
type PgClient = {
...
export declare const createSeedClient: (options?: SeedClientOptions & {
  dryRun?: boolean
  client?: PgClient
}) => Promise<SeedClient>`
  },
  inMemory() {
    return `
export declare const createSeedClient: (options?: statementStore: string[], options?: SeedClientOptions) => Promise<SeedClient>`
  },
}

function generateClientAdapterTypes(adapterName: AdapterName) {
  return [
    `
...
`,
    generateClientTypesByAdapter[adapterName](),
  ].join(os.EOL)
}
