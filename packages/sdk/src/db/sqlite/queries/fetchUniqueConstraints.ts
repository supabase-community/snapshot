...
type FetchUniqueConstraintsResultRaw = {
  tableName: string
  idxOrigin: 'c' | 'u' | 'pk'
  idxName: string
  idxPartial: 0 | 1
...
export async function fetchUniqueConstraints(client: SqliteClient) {
  const uniqueIndexesResponse =
    await queryNext<FetchUniqueConstraintsResultRaw>(FETCH_UNIQUE_CONSTRAINTS, {
      client,
    })
  ...
  const results: FetchUniqueConstraintsResult[] = Object.values(
    groupedResults
  ).map((value) => {
    return {
