import {
  ...
  createChinookSqliteTestDatabase,
} from '~/testing/createSqliteDb.js'
import { withDbClient } from './client.js'
import {
  basicIntrospectDatabase,
  introspectSqliteDatabase,
} from './introspectSqliteDatabase.js'
...
test('introspectDatabase should return basic database structure', async () => {
  ...
  const result = await withDbClient(introspectSqliteDatabase, {
    connString: connString.toString(),
  })
  expect(result).toEqual({
    server: {
...
test('basicIntrospectDatabase on chinook example database', async () => {
  const connString = await createChinookSqliteTestDatabase()
