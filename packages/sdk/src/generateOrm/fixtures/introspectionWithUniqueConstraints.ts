import { IntrospectedStructure } from '../../db/introspect/introspectDatabase.js'

export const introspectionWithConstraints: IntrospectedStructure = {
  schemas: ['pgboss', 'public'],
  tables: [
    {
      id: 'public.AccessToken',
      name: 'AccessToken',
      schema: 'public',
      partitioned: false,
      rows: 105,
      bytes: 73728,
      columns: [
        {
          id: 'public.AccessToken.id',
          name: 'id',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'AccessToken',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['p'],
        },
        {
          id: 'public.AccessToken.updatedAt',
          name: 'updatedAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'AccessToken',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.AccessToken.createdAt',
          name: 'createdAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'AccessToken',
          schema: 'public',
          nullable: false,
          default: 'CURRENT_TIMESTAMP',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.AccessToken.userId',
          name: 'userId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'AccessToken',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
        {
          id: 'public.AccessToken.userAgent',
          name: 'userAgent',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'AccessToken',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.AccessToken.type',
          name: 'type',
          type: 'AccessTokenType',
          typeId: 'public.AccessTokenType',
          table: 'AccessToken',
          schema: 'public',
          nullable: false,
          default: '\'CLI\'::"AccessTokenType"',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'E',
          constraints: [],
        },
        {
          id: 'public.AccessToken.name',
          name: 'name',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'AccessToken',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.AccessToken.hash',
          name: 'hash',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'AccessToken',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
      ],
      parents: [
        {
          id: 'AccessToken_userId_fkey',
          fkTable: 'public.AccessToken',
          keys: [
            {
              fkColumn: 'userId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.User',
        },
      ],
      children: [
        {
          id: 'ExecTask_accessTokenId_fkey',
          fkTable: 'public.ExecTask',
          keys: [
            {
              fkColumn: 'accessTokenId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.AccessToken',
        },
      ],
      primaryKeys: {
        tableId: 'public.AccessToken',
        schema: 'public',
        table: 'AccessToken',
        dirty: false,
        keys: [
          {
            name: 'id',
            type: 'text',
          },
        ],
      },
      constraints: [
        {
          tableId: 'public.AccessToken',
          schema: 'public',
          table: 'AccessToken',
          dirty: false,
          name: 'AccessToken_pkey',
          columns: ['id'],
        },
      ],
    },
    {
      id: 'public.AuditLog',
      name: 'AuditLog',
      schema: 'public',
      partitioned: false,
      rows: 29716,
      bytes: 5062656,
      columns: [
        {
          id: 'public.AuditLog.id',
          name: 'id',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'AuditLog',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['p'],
        },
        {
          id: 'public.AuditLog.createdAt',
          name: 'createdAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'AuditLog',
          schema: 'public',
          nullable: false,
          default: 'CURRENT_TIMESTAMP',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.AuditLog.action',
          name: 'action',
          type: 'AuditLogActions',
          typeId: 'public.AuditLogActions',
          table: 'AuditLog',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'E',
          constraints: [],
        },
        {
          id: 'public.AuditLog.data',
          name: 'data',
          type: 'jsonb',
          typeId: 'pg_catalog.jsonb',
          table: 'AuditLog',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'U',
          constraints: [],
        },
        {
          id: 'public.AuditLog.userId',
          name: 'userId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'AuditLog',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
        {
          id: 'public.AuditLog.organizationId',
          name: 'organizationId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'AuditLog',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
        {
          id: 'public.AuditLog.projectId',
          name: 'projectId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'AuditLog',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
      ],
      parents: [
        {
          id: 'AuditLog_organizationId_fkey',
          fkTable: 'public.AuditLog',
          keys: [
            {
              fkColumn: 'organizationId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.Organization',
        },
        {
          id: 'AuditLog_projectId_fkey',
          fkTable: 'public.AuditLog',
          keys: [
            {
              fkColumn: 'projectId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.Project',
        },
        {
          id: 'AuditLog_userId_fkey',
          fkTable: 'public.AuditLog',
          keys: [
            {
              fkColumn: 'userId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.User',
        },
      ],
      children: [],
      primaryKeys: {
        tableId: 'public.AuditLog',
        schema: 'public',
        table: 'AuditLog',
        dirty: false,
        keys: [
          {
            name: 'id',
            type: 'text',
          },
        ],
      },
      constraints: [
        {
          tableId: 'public.AuditLog',
          schema: 'public',
          table: 'AuditLog',
          dirty: false,
          name: 'AuditLog_pkey',
          columns: ['id'],
        },
      ],
    },
    {
      id: 'public.AwsConsumptionHistory',
      name: 'AwsConsumptionHistory',
      schema: 'public',
      partitioned: false,
      rows: 2387,
      bytes: 458752,
      columns: [
        {
          id: 'public.AwsConsumptionHistory.name',
          name: 'name',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'AwsConsumptionHistory',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['p'],
        },
        {
          id: 'public.AwsConsumptionHistory.startPeriod',
          name: 'startPeriod',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'AwsConsumptionHistory',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: ['p'],
        },
        {
          id: 'public.AwsConsumptionHistory.endPeriod',
          name: 'endPeriod',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'AwsConsumptionHistory',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: ['p'],
        },
        {
          id: 'public.AwsConsumptionHistory.awsStorageBytes',
          name: 'awsStorageBytes',
          type: 'int8',
          typeId: 'pg_catalog.int8',
          table: 'AwsConsumptionHistory',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: [],
        },
        {
          id: 'public.AwsConsumptionHistory.awsComputeTimeSeconds',
          name: 'awsComputeTimeSeconds',
          type: 'int8',
          typeId: 'pg_catalog.int8',
          table: 'AwsConsumptionHistory',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: [],
        },
        {
          id: 'public.AwsConsumptionHistory.awsDataTransferBytes',
          name: 'awsDataTransferBytes',
          type: 'int8',
          typeId: 'pg_catalog.int8',
          table: 'AwsConsumptionHistory',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: [],
        },
        {
          id: 'public.AwsConsumptionHistory.snapshotId',
          name: 'snapshotId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'AwsConsumptionHistory',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
        {
          id: 'public.AwsConsumptionHistory.projectId',
          name: 'projectId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'AwsConsumptionHistory',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
        {
          id: 'public.AwsConsumptionHistory.organizationId',
          name: 'organizationId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'AwsConsumptionHistory',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f', 'p'],
        },
      ],
      parents: [
        {
          id: 'AwsConsumptionHistory_organizationId_fkey',
          fkTable: 'public.AwsConsumptionHistory',
          keys: [
            {
              fkColumn: 'organizationId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.Organization',
        },
        {
          id: 'AwsConsumptionHistory_projectId_fkey',
          fkTable: 'public.AwsConsumptionHistory',
          keys: [
            {
              fkColumn: 'projectId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.Project',
        },
        {
          id: 'AwsConsumptionHistory_snapshotId_fkey',
          fkTable: 'public.AwsConsumptionHistory',
          keys: [
            {
              fkColumn: 'snapshotId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.Snapshot',
        },
      ],
      children: [],
      primaryKeys: {
        tableId: 'public.AwsConsumptionHistory',
        schema: 'public',
        table: 'AwsConsumptionHistory',
        dirty: false,
        keys: [
          {
            name: 'endPeriod',
            type: 'timestamp',
          },
          {
            name: 'name',
            type: 'text',
          },
          {
            name: 'organizationId',
            type: 'text',
          },
          {
            name: 'startPeriod',
            type: 'timestamp',
          },
        ],
      },
      constraints: [
        {
          tableId: 'public.AwsConsumptionHistory',
          schema: 'public',
          table: 'AwsConsumptionHistory',
          dirty: false,
          name: 'AwsConsumptionHistory_pkey',
          columns: ['endPeriod', 'name', 'organizationId', 'startPeriod'],
        },
      ],
    },
    {
      id: 'public.DatabaseProvider',
      name: 'DatabaseProvider',
      schema: 'public',
      partitioned: false,
      rows: 3,
      bytes: 40960,
      columns: [
        {
          id: 'public.DatabaseProvider.id',
          name: 'id',
          type: 'int4',
          typeId: 'pg_catalog.int4',
          table: 'DatabaseProvider',
          schema: 'public',
          nullable: false,
          default: 'nextval(\'"DatabaseProvider_id_seq"\'::regclass)',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: ['p'],
        },
        {
          id: 'public.DatabaseProvider.name',
          name: 'name',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'DatabaseProvider',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.DatabaseProvider.domain',
          name: 'domain',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'DatabaseProvider',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
      ],
      parents: [],
      children: [
        {
          id: 'DbConnection_databaseProviderId_fkey',
          fkTable: 'public.DbConnection',
          keys: [
            {
              fkColumn: 'databaseProviderId',
              fkType: 'int4',
              targetColumn: 'id',
              targetType: 'int4',
              nullable: true,
            },
          ],
          targetTable: 'public.DatabaseProvider',
        },
      ],
      primaryKeys: {
        tableId: 'public.DatabaseProvider',
        schema: 'public',
        table: 'DatabaseProvider',
        dirty: false,
        keys: [
          {
            name: 'id',
            type: 'int4',
          },
        ],
      },
      constraints: [
        {
          tableId: 'public.DatabaseProvider',
          schema: 'public',
          table: 'DatabaseProvider',
          dirty: false,
          name: 'DatabaseProvider_pkey',
          columns: ['id'],
        },
      ],
    },
    {
      id: 'public.DbConnection',
      name: 'DbConnection',
      schema: 'public',
      partitioned: false,
      rows: 29,
      bytes: 49152,
      columns: [
        {
          id: 'public.DbConnection.id',
          name: 'id',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'DbConnection',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['p'],
        },
        {
          id: 'public.DbConnection.name',
          name: 'name',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'DbConnection',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.DbConnection.ssl',
          name: 'ssl',
          type: 'bool',
          typeId: 'pg_catalog.bool',
          table: 'DbConnection',
          schema: 'public',
          nullable: false,
          default: 'true',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'B',
          constraints: [],
        },
        {
          id: 'public.DbConnection.connectionUrlHash',
          name: 'connectionUrlHash',
          type: 'jsonb',
          typeId: 'pg_catalog.jsonb',
          table: 'DbConnection',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'U',
          constraints: [],
        },
        {
          id: 'public.DbConnection.organizationId',
          name: 'organizationId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'DbConnection',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
        {
          id: 'public.DbConnection.databaseProviderId',
          name: 'databaseProviderId',
          type: 'int4',
          typeId: 'pg_catalog.int4',
          table: 'DbConnection',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: ['f'],
        },
      ],
      parents: [
        {
          id: 'DbConnection_databaseProviderId_fkey',
          fkTable: 'public.DbConnection',
          keys: [
            {
              fkColumn: 'databaseProviderId',
              fkType: 'int4',
              targetColumn: 'id',
              targetType: 'int4',
              nullable: true,
            },
          ],
          targetTable: 'public.DatabaseProvider',
        },
        {
          id: 'DbConnection_organizationId_fkey',
          fkTable: 'public.DbConnection',
          keys: [
            {
              fkColumn: 'organizationId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.Organization',
        },
      ],
      children: [
        {
          id: 'Project_dbConnectionId_fkey',
          fkTable: 'public.Project',
          keys: [
            {
              fkColumn: 'dbConnectionId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.DbConnection',
        },
        {
          id: 'Project_hostedDbUrlId_fkey',
          fkTable: 'public.Project',
          keys: [
            {
              fkColumn: 'hostedDbUrlId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.DbConnection',
        },
        {
          id: 'Snapshot_dbConnectionId_fkey',
          fkTable: 'public.Snapshot',
          keys: [
            {
              fkColumn: 'dbConnectionId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.DbConnection',
        },
      ],
      primaryKeys: {
        tableId: 'public.DbConnection',
        schema: 'public',
        table: 'DbConnection',
        dirty: false,
        keys: [
          {
            name: 'id',
            type: 'text',
          },
        ],
      },
      constraints: [
        {
          tableId: 'public.DbConnection',
          schema: 'public',
          table: 'DbConnection',
          dirty: false,
          name: 'DbConnection_pkey',
          columns: ['id'],
        },
      ],
    },
    {
      id: 'public.ExecTask',
      name: 'ExecTask',
      schema: 'public',
      partitioned: false,
      rows: 1027,
      bytes: 761856,
      columns: [
        {
          id: 'public.ExecTask.id',
          name: 'id',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'ExecTask',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['p'],
        },
        {
          id: 'public.ExecTask.command',
          name: 'command',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'ExecTask',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.ExecTask.env',
          name: 'env',
          type: 'jsonb',
          typeId: 'pg_catalog.jsonb',
          table: 'ExecTask',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'U',
          constraints: [],
        },
        {
          id: 'public.ExecTask.exitCode',
          name: 'exitCode',
          type: 'int4',
          typeId: 'pg_catalog.int4',
          table: 'ExecTask',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: [],
        },
        {
          id: 'public.ExecTask.createdAt',
          name: 'createdAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'ExecTask',
          schema: 'public',
          nullable: false,
          default: 'CURRENT_TIMESTAMP',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.ExecTask.updatedAt',
          name: 'updatedAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'ExecTask',
          schema: 'public',
          nullable: false,
          default: 'CURRENT_TIMESTAMP',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.ExecTask.projectId',
          name: 'projectId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'ExecTask',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
        {
          id: 'public.ExecTask.needsSourceDatabaseUrl',
          name: 'needsSourceDatabaseUrl',
          type: 'bool',
          typeId: 'pg_catalog.bool',
          table: 'ExecTask',
          schema: 'public',
          nullable: false,
          default: 'false',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'B',
          constraints: [],
        },
        {
          id: 'public.ExecTask.progress',
          name: 'progress',
          type: 'jsonb',
          typeId: 'pg_catalog.jsonb',
          table: 'ExecTask',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'U',
          constraints: [],
        },
        {
          id: 'public.ExecTask.endedAt',
          name: 'endedAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'ExecTask',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.ExecTask.arn',
          name: 'arn',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'ExecTask',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.ExecTask.accessTokenId',
          name: 'accessTokenId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'ExecTask',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
        {
          id: 'public.ExecTask.lastNotifiedAt',
          name: 'lastNotifiedAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'ExecTask',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
      ],
      parents: [
        {
          id: 'ExecTask_accessTokenId_fkey',
          fkTable: 'public.ExecTask',
          keys: [
            {
              fkColumn: 'accessTokenId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.AccessToken',
        },
        {
          id: 'ExecTask_projectId_fkey',
          fkTable: 'public.ExecTask',
          keys: [
            {
              fkColumn: 'projectId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.Project',
        },
      ],
      children: [
        {
          id: 'Snapshot_execTaskId_fkey',
          fkTable: 'public.Snapshot',
          keys: [
            {
              fkColumn: 'execTaskId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.ExecTask',
        },
      ],
      primaryKeys: {
        tableId: 'public.ExecTask',
        schema: 'public',
        table: 'ExecTask',
        dirty: false,
        keys: [
          {
            name: 'id',
            type: 'text',
          },
        ],
      },
      constraints: [
        {
          tableId: 'public.ExecTask',
          schema: 'public',
          table: 'ExecTask',
          dirty: false,
          name: 'ExecTask_pkey',
          columns: ['id'],
        },
      ],
    },
    {
      id: 'public.InviteToken',
      name: 'InviteToken',
      schema: 'public',
      partitioned: false,
      rows: 24,
      bytes: 40960,
      columns: [
        {
          id: 'public.InviteToken.token',
          name: 'token',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'InviteToken',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['p'],
        },
        {
          id: 'public.InviteToken.createdAt',
          name: 'createdAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'InviteToken',
          schema: 'public',
          nullable: false,
          default: 'CURRENT_TIMESTAMP',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.InviteToken.updatedAt',
          name: 'updatedAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'InviteToken',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.InviteToken.createdByUserId',
          name: 'createdByUserId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'InviteToken',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
        {
          id: 'public.InviteToken.usedByMemberId',
          name: 'usedByMemberId',
          type: 'int4',
          typeId: 'pg_catalog.int4',
          table: 'InviteToken',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: ['f'],
        },
        {
          id: 'public.InviteToken.organizationId',
          name: 'organizationId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'InviteToken',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
        {
          id: 'public.InviteToken.expiresAt',
          name: 'expiresAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'InviteToken',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
      ],
      parents: [
        {
          id: 'InviteToken_usedByMemberId_fkey',
          fkTable: 'public.InviteToken',
          keys: [
            {
              fkColumn: 'usedByMemberId',
              fkType: 'int4',
              targetColumn: 'id',
              targetType: 'int4',
              nullable: true,
            },
          ],
          targetTable: 'public.Member',
        },
        {
          id: 'InviteToken_organizationId_fkey',
          fkTable: 'public.InviteToken',
          keys: [
            {
              fkColumn: 'organizationId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.Organization',
        },
        {
          id: 'InviteToken_createdByUserId_fkey',
          fkTable: 'public.InviteToken',
          keys: [
            {
              fkColumn: 'createdByUserId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.User',
        },
      ],
      children: [],
      primaryKeys: {
        tableId: 'public.InviteToken',
        schema: 'public',
        table: 'InviteToken',
        dirty: false,
        keys: [
          {
            name: 'token',
            type: 'text',
          },
        ],
      },
      constraints: [
        {
          tableId: 'public.InviteToken',
          schema: 'public',
          table: 'InviteToken',
          dirty: false,
          name: 'InviteToken_pkey',
          columns: ['token'],
        },
      ],
    },
    {
      id: 'public.Member',
      name: 'Member',
      schema: 'public',
      partitioned: false,
      rows: 17,
      bytes: 40960,
      columns: [
        {
          id: 'public.Member.role',
          name: 'role',
          type: 'MemberRole',
          typeId: 'public.MemberRole',
          table: 'Member',
          schema: 'public',
          nullable: false,
          default: '\'MEMBER\'::"MemberRole"',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'E',
          constraints: [],
        },
        {
          id: 'public.Member.organizationId',
          name: 'organizationId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Member',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
        {
          id: 'public.Member.userId',
          name: 'userId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Member',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
        {
          id: 'public.Member.id',
          name: 'id',
          type: 'int4',
          typeId: 'pg_catalog.int4',
          table: 'Member',
          schema: 'public',
          nullable: false,
          default: 'nextval(\'"Member_id_seq"\'::regclass)',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: ['p'],
        },
        {
          id: 'public.Member.createdAt',
          name: 'createdAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'Member',
          schema: 'public',
          nullable: false,
          default: 'CURRENT_TIMESTAMP',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.Member.updatedAt',
          name: 'updatedAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'Member',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
      ],
      parents: [
        {
          id: 'Member_organizationId_fkey',
          fkTable: 'public.Member',
          keys: [
            {
              fkColumn: 'organizationId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.Organization',
        },
        {
          id: 'Member_userId_fkey',
          fkTable: 'public.Member',
          keys: [
            {
              fkColumn: 'userId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.User',
        },
      ],
      children: [
        {
          id: 'InviteToken_usedByMemberId_fkey',
          fkTable: 'public.InviteToken',
          keys: [
            {
              fkColumn: 'usedByMemberId',
              fkType: 'int4',
              targetColumn: 'id',
              targetType: 'int4',
              nullable: true,
            },
          ],
          targetTable: 'public.Member',
        },
      ],
      primaryKeys: {
        tableId: 'public.Member',
        schema: 'public',
        table: 'Member',
        dirty: false,
        keys: [
          {
            name: 'id',
            type: 'int4',
          },
        ],
      },
      constraints: [
        {
          tableId: 'public.Member',
          schema: 'public',
          table: 'Member',
          dirty: false,
          name: 'Member_pkey',
          columns: ['id'],
        },
      ],
    },
    {
      id: 'public.NeonConsumptionHistory',
      name: 'NeonConsumptionHistory',
      schema: 'public',
      partitioned: false,
      rows: 899,
      bytes: 221184,
      columns: [
        {
          id: 'public.NeonConsumptionHistory.name',
          name: 'name',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'NeonConsumptionHistory',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['p'],
        },
        {
          id: 'public.NeonConsumptionHistory.startPeriod',
          name: 'startPeriod',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'NeonConsumptionHistory',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: ['p'],
        },
        {
          id: 'public.NeonConsumptionHistory.endPeriod',
          name: 'endPeriod',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'NeonConsumptionHistory',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: ['p'],
        },
        {
          id: 'public.NeonConsumptionHistory.neonDataStorageBytesHour',
          name: 'neonDataStorageBytesHour',
          type: 'int8',
          typeId: 'pg_catalog.int8',
          table: 'NeonConsumptionHistory',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: [],
        },
        {
          id: 'public.NeonConsumptionHistory.neonSyntheticStorageSize',
          name: 'neonSyntheticStorageSize',
          type: 'int8',
          typeId: 'pg_catalog.int8',
          table: 'NeonConsumptionHistory',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: [],
        },
        {
          id: 'public.NeonConsumptionHistory.neonDataTransferBytes',
          name: 'neonDataTransferBytes',
          type: 'int8',
          typeId: 'pg_catalog.int8',
          table: 'NeonConsumptionHistory',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: [],
        },
        {
          id: 'public.NeonConsumptionHistory.neonWrittenDataBytes',
          name: 'neonWrittenDataBytes',
          type: 'int8',
          typeId: 'pg_catalog.int8',
          table: 'NeonConsumptionHistory',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: [],
        },
        {
          id: 'public.NeonConsumptionHistory.neonComputeTimeSeconds',
          name: 'neonComputeTimeSeconds',
          type: 'int8',
          typeId: 'pg_catalog.int8',
          table: 'NeonConsumptionHistory',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: [],
        },
        {
          id: 'public.NeonConsumptionHistory.neonActiveTimeSeconds',
          name: 'neonActiveTimeSeconds',
          type: 'int8',
          typeId: 'pg_catalog.int8',
          table: 'NeonConsumptionHistory',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: [],
        },
        {
          id: 'public.NeonConsumptionHistory.snapshotId',
          name: 'snapshotId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'NeonConsumptionHistory',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
        {
          id: 'public.NeonConsumptionHistory.projectId',
          name: 'projectId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'NeonConsumptionHistory',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
        {
          id: 'public.NeonConsumptionHistory.organizationId',
          name: 'organizationId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'NeonConsumptionHistory',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f', 'p'],
        },
      ],
      parents: [
        {
          id: 'NeonConsumptionHistory_organizationId_fkey',
          fkTable: 'public.NeonConsumptionHistory',
          keys: [
            {
              fkColumn: 'organizationId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.Organization',
        },
        {
          id: 'NeonConsumptionHistory_projectId_fkey',
          fkTable: 'public.NeonConsumptionHistory',
          keys: [
            {
              fkColumn: 'projectId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.Project',
        },
        {
          id: 'NeonConsumptionHistory_snapshotId_fkey',
          fkTable: 'public.NeonConsumptionHistory',
          keys: [
            {
              fkColumn: 'snapshotId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.Snapshot',
        },
      ],
      children: [],
      primaryKeys: {
        tableId: 'public.NeonConsumptionHistory',
        schema: 'public',
        table: 'NeonConsumptionHistory',
        dirty: false,
        keys: [
          {
            name: 'endPeriod',
            type: 'timestamp',
          },
          {
            name: 'name',
            type: 'text',
          },
          {
            name: 'organizationId',
            type: 'text',
          },
          {
            name: 'startPeriod',
            type: 'timestamp',
          },
        ],
      },
      constraints: [
        {
          tableId: 'public.NeonConsumptionHistory',
          schema: 'public',
          table: 'NeonConsumptionHistory',
          dirty: false,
          name: 'NeonConsumptionHistory_pkey',
          columns: ['endPeriod', 'name', 'organizationId', 'startPeriod'],
        },
      ],
    },
    {
      id: 'public.NeonProject',
      name: 'NeonProject',
      schema: 'public',
      partitioned: false,
      rows: 11,
      bytes: 40960,
      columns: [
        {
          id: 'public.NeonProject.id',
          name: 'id',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'NeonProject',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['p'],
        },
        {
          id: 'public.NeonProject.createdAt',
          name: 'createdAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'NeonProject',
          schema: 'public',
          nullable: false,
          default: 'CURRENT_TIMESTAMP',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.NeonProject.updatedAt',
          name: 'updatedAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'NeonProject',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.NeonProject.neonProjectId',
          name: 'neonProjectId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'NeonProject',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.NeonProject.snapshotId',
          name: 'snapshotId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'NeonProject',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
        {
          id: 'public.NeonProject.connectionUrlHash',
          name: 'connectionUrlHash',
          type: 'jsonb',
          typeId: 'pg_catalog.jsonb',
          table: 'NeonProject',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'U',
          constraints: [],
        },
        {
          id: 'public.NeonProject.projectId',
          name: 'projectId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'NeonProject',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
        {
          id: 'public.NeonProject.status',
          name: 'status',
          type: 'PreviewDatabaseStatus',
          typeId: 'public.PreviewDatabaseStatus',
          table: 'NeonProject',
          schema: 'public',
          nullable: false,
          default: '\'IN_PROGRESS\'::"PreviewDatabaseStatus"',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'E',
          constraints: [],
        },
      ],
      parents: [
        {
          id: 'NeonProject_projectId_fkey',
          fkTable: 'public.NeonProject',
          keys: [
            {
              fkColumn: 'projectId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.Project',
        },
        {
          id: 'NeonProject_snapshotId_fkey',
          fkTable: 'public.NeonProject',
          keys: [
            {
              fkColumn: 'snapshotId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.Snapshot',
        },
      ],
      children: [
        {
          id: 'PreviewDatabase_neonProjectId_fkey',
          fkTable: 'public.PreviewDatabase',
          keys: [
            {
              fkColumn: 'neonProjectId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.NeonProject',
        },
      ],
      primaryKeys: {
        tableId: 'public.NeonProject',
        schema: 'public',
        table: 'NeonProject',
        dirty: false,
        keys: [
          {
            name: 'id',
            type: 'text',
          },
        ],
      },
      constraints: [
        {
          tableId: 'public.NeonProject',
          schema: 'public',
          table: 'NeonProject',
          dirty: false,
          name: 'NeonProject_pkey',
          columns: ['id'],
        },
      ],
    },
    {
      id: 'public.Organization',
      name: 'Organization',
      schema: 'public',
      partitioned: false,
      rows: 6,
      bytes: 40960,
      columns: [
        {
          id: 'public.Organization.id',
          name: 'id',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Organization',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['p'],
        },
        {
          id: 'public.Organization.name',
          name: 'name',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Organization',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.Organization.pricingPlanId',
          name: 'pricingPlanId',
          type: 'int4',
          typeId: 'pg_catalog.int4',
          table: 'Organization',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: ['f'],
        },
        {
          id: 'public.Organization.subscriptionData',
          name: 'subscriptionData',
          type: 'jsonb',
          typeId: 'pg_catalog.jsonb',
          table: 'Organization',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'U',
          constraints: [],
        },
        {
          id: 'public.Organization.createdAt',
          name: 'createdAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'Organization',
          schema: 'public',
          nullable: false,
          default: 'CURRENT_TIMESTAMP',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.Organization.updatedAt',
          name: 'updatedAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'Organization',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.Organization.deleted',
          name: 'deleted',
          type: 'bool',
          typeId: 'pg_catalog.bool',
          table: 'Organization',
          schema: 'public',
          nullable: false,
          default: 'false',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'B',
          constraints: [],
        },
      ],
      parents: [
        {
          id: 'Organization_pricingPlanId_fkey',
          fkTable: 'public.Organization',
          keys: [
            {
              fkColumn: 'pricingPlanId',
              fkType: 'int4',
              targetColumn: 'id',
              targetType: 'int4',
              nullable: true,
            },
          ],
          targetTable: 'public.PricingPlan',
        },
      ],
      children: [
        {
          id: 'AuditLog_organizationId_fkey',
          fkTable: 'public.AuditLog',
          keys: [
            {
              fkColumn: 'organizationId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.Organization',
        },
        {
          id: 'AwsConsumptionHistory_organizationId_fkey',
          fkTable: 'public.AwsConsumptionHistory',
          keys: [
            {
              fkColumn: 'organizationId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.Organization',
        },
        {
          id: 'DbConnection_organizationId_fkey',
          fkTable: 'public.DbConnection',
          keys: [
            {
              fkColumn: 'organizationId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.Organization',
        },
        {
          id: 'InviteToken_organizationId_fkey',
          fkTable: 'public.InviteToken',
          keys: [
            {
              fkColumn: 'organizationId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.Organization',
        },
        {
          id: 'Member_organizationId_fkey',
          fkTable: 'public.Member',
          keys: [
            {
              fkColumn: 'organizationId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.Organization',
        },
        {
          id: 'NeonConsumptionHistory_organizationId_fkey',
          fkTable: 'public.NeonConsumptionHistory',
          keys: [
            {
              fkColumn: 'organizationId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.Organization',
        },
        {
          id: 'Project_organizationId_fkey',
          fkTable: 'public.Project',
          keys: [
            {
              fkColumn: 'organizationId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.Organization',
        },
        {
          id: 'Snapshot_organizationId_fkey',
          fkTable: 'public.Snapshot',
          keys: [
            {
              fkColumn: 'organizationId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.Organization',
        },
        {
          id: 'Table_organizationId_fkey',
          fkTable: 'public.Table',
          keys: [
            {
              fkColumn: 'organizationId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.Organization',
        },
      ],
      primaryKeys: {
        tableId: 'public.Organization',
        schema: 'public',
        table: 'Organization',
        dirty: false,
        keys: [
          {
            name: 'id',
            type: 'text',
          },
        ],
      },
      constraints: [
        {
          tableId: 'public.Organization',
          schema: 'public',
          table: 'Organization',
          dirty: false,
          name: 'Organization_pkey',
          columns: ['id'],
        },
      ],
    },
    {
      id: 'public.PredictionDataSet',
      name: 'PredictionDataSet',
      schema: 'public',
      partitioned: false,
      rows: 4053,
      bytes: 630784,
      columns: [
        {
          id: 'public.PredictionDataSet.id',
          name: 'id',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'PredictionDataSet',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['p'],
        },
        {
          id: 'public.PredictionDataSet.createdAt',
          name: 'createdAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'PredictionDataSet',
          schema: 'public',
          nullable: false,
          default: 'CURRENT_TIMESTAMP',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.PredictionDataSet.updatedAt',
          name: 'updatedAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'PredictionDataSet',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.PredictionDataSet.input',
          name: 'input',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'PredictionDataSet',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.PredictionDataSet.context',
          name: 'context',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'PredictionDataSet',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.PredictionDataSet.shape',
          name: 'shape',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'PredictionDataSet',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.PredictionDataSet.contextSkipTraining',
          name: 'contextSkipTraining',
          type: 'bool',
          typeId: 'pg_catalog.bool',
          table: 'PredictionDataSet',
          schema: 'public',
          nullable: false,
          default: 'false',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'B',
          constraints: [],
        },
        {
          id: 'public.PredictionDataSet.shapeSkipTraining',
          name: 'shapeSkipTraining',
          type: 'bool',
          typeId: 'pg_catalog.bool',
          table: 'PredictionDataSet',
          schema: 'public',
          nullable: false,
          default: 'false',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'B',
          constraints: [],
        },
      ],
      parents: [],
      children: [],
      primaryKeys: {
        tableId: 'public.PredictionDataSet',
        schema: 'public',
        table: 'PredictionDataSet',
        dirty: false,
        keys: [
          {
            name: 'id',
            type: 'text',
          },
        ],
      },
      constraints: [
        {
          tableId: 'public.PredictionDataSet',
          schema: 'public',
          table: 'PredictionDataSet',
          dirty: false,
          name: 'PredictionDataSet_pkey',
          columns: ['id'],
        },
      ],
    },
    {
      id: 'public.PredictionJob',
      name: 'PredictionJob',
      schema: 'public',
      partitioned: false,
      rows: 0,
      bytes: 8192,
      columns: [
        {
          id: 'public.PredictionJob.id',
          name: 'id',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'PredictionJob',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['p'],
        },
        {
          id: 'public.PredictionJob.engine',
          name: 'engine',
          type: 'PredictionsEngine',
          typeId: 'public.PredictionsEngine',
          table: 'PredictionJob',
          schema: 'public',
          nullable: false,
          default: '\'FINETUNED_BERT\'::"PredictionsEngine"',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'E',
          constraints: [],
        },
        {
          id: 'public.PredictionJob.createdAt',
          name: 'createdAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'PredictionJob',
          schema: 'public',
          nullable: false,
          default: 'CURRENT_TIMESTAMP',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.PredictionJob.updatedAt',
          name: 'updatedAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'PredictionJob',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.PredictionJob.endedAt',
          name: 'endedAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'PredictionJob',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.PredictionJob.rawInput',
          name: 'rawInput',
          type: 'jsonb',
          typeId: 'pg_catalog.jsonb',
          table: 'PredictionJob',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'U',
          constraints: [],
        },
        {
          id: 'public.PredictionJob.engineInput',
          name: 'engineInput',
          type: 'jsonb',
          typeId: 'pg_catalog.jsonb',
          table: 'PredictionJob',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'U',
          constraints: [],
        },
        {
          id: 'public.PredictionJob.progress',
          name: 'progress',
          type: 'jsonb',
          typeId: 'pg_catalog.jsonb',
          table: 'PredictionJob',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'U',
          constraints: [],
        },
        {
          id: 'public.PredictionJob.engineOptions',
          name: 'engineOptions',
          type: 'jsonb',
          typeId: 'pg_catalog.jsonb',
          table: 'PredictionJob',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'U',
          constraints: [],
        },
      ],
      parents: [],
      children: [
        {
          id: 'Project_predictionJobId_fkey',
          fkTable: 'public.Project',
          keys: [
            {
              fkColumn: 'predictionJobId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.PredictionJob',
        },
      ],
      primaryKeys: {
        tableId: 'public.PredictionJob',
        schema: 'public',
        table: 'PredictionJob',
        dirty: false,
        keys: [
          {
            name: 'id',
            type: 'text',
          },
        ],
      },
      constraints: [
        {
          tableId: 'public.PredictionJob',
          schema: 'public',
          table: 'PredictionJob',
          dirty: false,
          name: 'PredictionJob_pkey',
          columns: ['id'],
        },
      ],
    },
    {
      id: 'public.PreviewDatabase',
      name: 'PreviewDatabase',
      schema: 'public',
      partitioned: false,
      rows: 15,
      bytes: 40960,
      columns: [
        {
          id: 'public.PreviewDatabase.id',
          name: 'id',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'PreviewDatabase',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['p'],
        },
        {
          id: 'public.PreviewDatabase.createdAt',
          name: 'createdAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'PreviewDatabase',
          schema: 'public',
          nullable: false,
          default: 'CURRENT_TIMESTAMP',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.PreviewDatabase.updatedAt',
          name: 'updatedAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'PreviewDatabase',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.PreviewDatabase.name',
          name: 'name',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'PreviewDatabase',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.PreviewDatabase.neonBranchId',
          name: 'neonBranchId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'PreviewDatabase',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.PreviewDatabase.neonProjectId',
          name: 'neonProjectId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'PreviewDatabase',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
        {
          id: 'public.PreviewDatabase.connectionUrlHash',
          name: 'connectionUrlHash',
          type: 'jsonb',
          typeId: 'pg_catalog.jsonb',
          table: 'PreviewDatabase',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'U',
          constraints: [],
        },
        {
          id: 'public.PreviewDatabase.status',
          name: 'status',
          type: 'PreviewDatabaseStatus',
          typeId: 'public.PreviewDatabaseStatus',
          table: 'PreviewDatabase',
          schema: 'public',
          nullable: false,
          default: '\'IN_PROGRESS\'::"PreviewDatabaseStatus"',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'E',
          constraints: [],
        },
      ],
      parents: [
        {
          id: 'PreviewDatabase_neonProjectId_fkey',
          fkTable: 'public.PreviewDatabase',
          keys: [
            {
              fkColumn: 'neonProjectId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.NeonProject',
        },
      ],
      children: [],
      primaryKeys: {
        tableId: 'public.PreviewDatabase',
        schema: 'public',
        table: 'PreviewDatabase',
        dirty: false,
        keys: [
          {
            name: 'id',
            type: 'text',
          },
        ],
      },
      constraints: [
        {
          tableId: 'public.PreviewDatabase',
          schema: 'public',
          table: 'PreviewDatabase',
          dirty: false,
          name: 'PreviewDatabase_pkey',
          columns: ['id'],
        },
      ],
    },
    {
      id: 'public.PricingPlan',
      name: 'PricingPlan',
      schema: 'public',
      partitioned: false,
      rows: 1,
      bytes: 40960,
      columns: [
        {
          id: 'public.PricingPlan.id',
          name: 'id',
          type: 'int4',
          typeId: 'pg_catalog.int4',
          table: 'PricingPlan',
          schema: 'public',
          nullable: false,
          default: 'nextval(\'"PricingPlan_id_seq"\'::regclass)',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: ['p'],
        },
        {
          id: 'public.PricingPlan.name',
          name: 'name',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'PricingPlan',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.PricingPlan.amount',
          name: 'amount',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'PricingPlan',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.PricingPlan.isDefault',
          name: 'isDefault',
          type: 'bool',
          typeId: 'pg_catalog.bool',
          table: 'PricingPlan',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'B',
          constraints: [],
        },
        {
          id: 'public.PricingPlan.storageLimit',
          name: 'storageLimit',
          type: 'int4',
          typeId: 'pg_catalog.int4',
          table: 'PricingPlan',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: [],
        },
        {
          id: 'public.PricingPlan.processLimit',
          name: 'processLimit',
          type: 'int4',
          typeId: 'pg_catalog.int4',
          table: 'PricingPlan',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: [],
        },
        {
          id: 'public.PricingPlan.restoreLimit',
          name: 'restoreLimit',
          type: 'int4',
          typeId: 'pg_catalog.int4',
          table: 'PricingPlan',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: [],
        },
        {
          id: 'public.PricingPlan.productId',
          name: 'productId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'PricingPlan',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
      ],
      parents: [],
      children: [
        {
          id: 'Organization_pricingPlanId_fkey',
          fkTable: 'public.Organization',
          keys: [
            {
              fkColumn: 'pricingPlanId',
              fkType: 'int4',
              targetColumn: 'id',
              targetType: 'int4',
              nullable: true,
            },
          ],
          targetTable: 'public.PricingPlan',
        },
      ],
      primaryKeys: {
        tableId: 'public.PricingPlan',
        schema: 'public',
        table: 'PricingPlan',
        dirty: false,
        keys: [
          {
            name: 'id',
            type: 'int4',
          },
        ],
      },
      constraints: [
        {
          tableId: 'public.PricingPlan',
          schema: 'public',
          table: 'PricingPlan',
          dirty: false,
          name: 'PricingPlan_pkey',
          columns: ['id'],
        },
      ],
    },
    {
      id: 'public.Project',
      name: 'Project',
      schema: 'public',
      partitioned: false,
      rows: 41,
      bytes: 81920,
      columns: [
        {
          id: 'public.Project.name',
          name: 'name',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Project',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.Project.organizationId',
          name: 'organizationId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Project',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
        {
          id: 'public.Project.dbConnectionId',
          name: 'dbConnectionId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Project',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
        {
          id: 'public.Project.id',
          name: 'id',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Project',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['p'],
        },
        {
          id: 'public.Project.createdAt',
          name: 'createdAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'Project',
          schema: 'public',
          nullable: false,
          default: 'CURRENT_TIMESTAMP',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.Project.updatedAt',
          name: 'updatedAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'Project',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.Project.dbInfo',
          name: 'dbInfo',
          type: 'jsonb',
          typeId: 'pg_catalog.jsonb',
          table: 'Project',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'U',
          constraints: [],
        },
        {
          id: 'public.Project.dbInfoLastUpdate',
          name: 'dbInfoLastUpdate',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'Project',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.Project.deleted',
          name: 'deleted',
          type: 'bool',
          typeId: 'pg_catalog.bool',
          table: 'Project',
          schema: 'public',
          nullable: false,
          default: 'false',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'B',
          constraints: [],
        },
        {
          id: 'public.Project.autoDeleteDays',
          name: 'autoDeleteDays',
          type: 'int4',
          typeId: 'pg_catalog.int4',
          table: 'Project',
          schema: 'public',
          nullable: true,
          default: '7',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: [],
        },
        {
          id: 'public.Project.snapshotConfig',
          name: 'snapshotConfig',
          type: 'jsonb',
          typeId: 'pg_catalog.jsonb',
          table: 'Project',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'U',
          constraints: [],
        },
        {
          id: 'public.Project.schedule',
          name: 'schedule',
          type: 'jsonb',
          typeId: 'pg_catalog.jsonb',
          table: 'Project',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'U',
          constraints: [],
        },
        {
          id: 'public.Project.runTaskOptions',
          name: 'runTaskOptions',
          type: 'jsonb',
          typeId: 'pg_catalog.jsonb',
          table: 'Project',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'U',
          constraints: [],
        },
        {
          id: 'public.Project.hostedDbUrlId',
          name: 'hostedDbUrlId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Project',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
        {
          id: 'public.Project.hostedDbRegion',
          name: 'hostedDbRegion',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Project',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.Project.scheduleTags',
          name: 'scheduleTags',
          type: 'text[]',
          typeId: 'pg_catalog._text',
          table: 'Project',
          schema: 'public',
          nullable: true,
          default: "ARRAY['main'::text]",
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'A',
          constraints: [],
        },
        {
          id: 'public.Project.previewDatabaseRegion',
          name: 'previewDatabaseRegion',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Project',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.Project.predictionJobId',
          name: 'predictionJobId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Project',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
        {
          id: 'public.Project.supabaseProjectId',
          name: 'supabaseProjectId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Project',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
        {
          id: 'public.Project.preseedPreviewDatabases',
          name: 'preseedPreviewDatabases',
          type: 'bool',
          typeId: 'pg_catalog.bool',
          table: 'Project',
          schema: 'public',
          nullable: false,
          default: 'false',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'B',
          constraints: [],
        },
      ],
      parents: [
        {
          id: 'Project_dbConnectionId_fkey',
          fkTable: 'public.Project',
          keys: [
            {
              fkColumn: 'dbConnectionId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.DbConnection',
        },
        {
          id: 'Project_hostedDbUrlId_fkey',
          fkTable: 'public.Project',
          keys: [
            {
              fkColumn: 'hostedDbUrlId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.DbConnection',
        },
        {
          id: 'Project_organizationId_fkey',
          fkTable: 'public.Project',
          keys: [
            {
              fkColumn: 'organizationId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.Organization',
        },
        {
          id: 'Project_predictionJobId_fkey',
          fkTable: 'public.Project',
          keys: [
            {
              fkColumn: 'predictionJobId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.PredictionJob',
        },
        {
          id: 'Project_supabaseProjectId_fkey',
          fkTable: 'public.Project',
          keys: [
            {
              fkColumn: 'supabaseProjectId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.SupabaseProject',
        },
      ],
      children: [
        {
          id: 'AuditLog_projectId_fkey',
          fkTable: 'public.AuditLog',
          keys: [
            {
              fkColumn: 'projectId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.Project',
        },
        {
          id: 'AwsConsumptionHistory_projectId_fkey',
          fkTable: 'public.AwsConsumptionHistory',
          keys: [
            {
              fkColumn: 'projectId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.Project',
        },
        {
          id: 'ExecTask_projectId_fkey',
          fkTable: 'public.ExecTask',
          keys: [
            {
              fkColumn: 'projectId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.Project',
        },
        {
          id: 'NeonConsumptionHistory_projectId_fkey',
          fkTable: 'public.NeonConsumptionHistory',
          keys: [
            {
              fkColumn: 'projectId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.Project',
        },
        {
          id: 'NeonProject_projectId_fkey',
          fkTable: 'public.NeonProject',
          keys: [
            {
              fkColumn: 'projectId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.Project',
        },
        {
          id: 'ShapePredictionOverride_projectId_fkey',
          fkTable: 'public.ShapePredictionOverride',
          keys: [
            {
              fkColumn: 'projectId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.Project',
        },
        {
          id: 'Snapshot_projectId_fkey',
          fkTable: 'public.Snapshot',
          keys: [
            {
              fkColumn: 'projectId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.Project',
        },
      ],
      primaryKeys: {
        tableId: 'public.Project',
        schema: 'public',
        table: 'Project',
        dirty: false,
        keys: [
          {
            name: 'id',
            type: 'text',
          },
        ],
      },
      constraints: [
        {
          tableId: 'public.Project',
          schema: 'public',
          table: 'Project',
          dirty: false,
          name: 'Project_pkey',
          columns: ['id'],
        },
      ],
    },
    {
      id: 'public.ReleaseVersion',
      name: 'ReleaseVersion',
      schema: 'public',
      partitioned: false,
      rows: 321,
      bytes: 57344,
      columns: [
        {
          id: 'public.ReleaseVersion.version',
          name: 'version',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'ReleaseVersion',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['p'],
        },
        {
          id: 'public.ReleaseVersion.channel',
          name: 'channel',
          type: 'ReleaseChannel',
          typeId: 'public.ReleaseChannel',
          table: 'ReleaseVersion',
          schema: 'public',
          nullable: false,
          default: '\'PUBLIC\'::"ReleaseChannel"',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'E',
          constraints: [],
        },
        {
          id: 'public.ReleaseVersion.forceUpgrade',
          name: 'forceUpgrade',
          type: 'bool',
          typeId: 'pg_catalog.bool',
          table: 'ReleaseVersion',
          schema: 'public',
          nullable: false,
          default: 'false',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'B',
          constraints: [],
        },
        {
          id: 'public.ReleaseVersion.releaseDate',
          name: 'releaseDate',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'ReleaseVersion',
          schema: 'public',
          nullable: false,
          default: 'CURRENT_TIMESTAMP',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.ReleaseVersion.userId',
          name: 'userId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'ReleaseVersion',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
      ],
      parents: [
        {
          id: 'ReleaseVersion_userId_fkey',
          fkTable: 'public.ReleaseVersion',
          keys: [
            {
              fkColumn: 'userId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.User',
        },
      ],
      children: [],
      primaryKeys: {
        tableId: 'public.ReleaseVersion',
        schema: 'public',
        table: 'ReleaseVersion',
        dirty: false,
        keys: [
          {
            name: 'version',
            type: 'text',
          },
        ],
      },
      constraints: [
        {
          tableId: 'public.ReleaseVersion',
          schema: 'public',
          table: 'ReleaseVersion',
          dirty: false,
          name: 'ReleaseVersion_pkey',
          columns: ['version'],
        },
      ],
    },
    {
      id: 'public.ShapePredictionOverride',
      name: 'ShapePredictionOverride',
      schema: 'public',
      partitioned: false,
      rows: 0,
      bytes: 0,
      columns: [
        {
          id: 'public.ShapePredictionOverride.id',
          name: 'id',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'ShapePredictionOverride',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['p'],
        },
        {
          id: 'public.ShapePredictionOverride.createdAt',
          name: 'createdAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'ShapePredictionOverride',
          schema: 'public',
          nullable: false,
          default: 'CURRENT_TIMESTAMP',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.ShapePredictionOverride.updatedAt',
          name: 'updatedAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'ShapePredictionOverride',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.ShapePredictionOverride.input',
          name: 'input',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'ShapePredictionOverride',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.ShapePredictionOverride.shape',
          name: 'shape',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'ShapePredictionOverride',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.ShapePredictionOverride.context',
          name: 'context',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'ShapePredictionOverride',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.ShapePredictionOverride.projectId',
          name: 'projectId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'ShapePredictionOverride',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
      ],
      parents: [
        {
          id: 'ShapePredictionOverride_projectId_fkey',
          fkTable: 'public.ShapePredictionOverride',
          keys: [
            {
              fkColumn: 'projectId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.Project',
        },
      ],
      children: [],
      primaryKeys: {
        tableId: 'public.ShapePredictionOverride',
        schema: 'public',
        table: 'ShapePredictionOverride',
        dirty: false,
        keys: [
          {
            name: 'id',
            type: 'text',
          },
        ],
      },
      constraints: [
        {
          tableId: 'public.ShapePredictionOverride',
          schema: 'public',
          table: 'ShapePredictionOverride',
          dirty: false,
          name: 'ShapePredictionOverride_pkey',
          columns: ['id'],
        },
      ],
    },
    {
      id: 'public.ShapePredictionStore',
      name: 'ShapePredictionStore',
      schema: 'public',
      partitioned: false,
      rows: 53929,
      bytes: 9306112,
      columns: [
        {
          id: 'public.ShapePredictionStore.id',
          name: 'id',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'ShapePredictionStore',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['p'],
        },
        {
          id: 'public.ShapePredictionStore.createdAt',
          name: 'createdAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'ShapePredictionStore',
          schema: 'public',
          nullable: false,
          default: 'CURRENT_TIMESTAMP',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.ShapePredictionStore.updatedAt',
          name: 'updatedAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'ShapePredictionStore',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.ShapePredictionStore.input',
          name: 'input',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'ShapePredictionStore',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.ShapePredictionStore.predictedLabel',
          name: 'predictedLabel',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'ShapePredictionStore',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.ShapePredictionStore.confidence',
          name: 'confidence',
          type: 'float8',
          typeId: 'pg_catalog.float8',
          table: 'ShapePredictionStore',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: [],
        },
        {
          id: 'public.ShapePredictionStore.overrideLabel',
          name: 'overrideLabel',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'ShapePredictionStore',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.ShapePredictionStore.confidenceContext',
          name: 'confidenceContext',
          type: 'float8',
          typeId: 'pg_catalog.float8',
          table: 'ShapePredictionStore',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: [],
        },
        {
          id: 'public.ShapePredictionStore.overrideContext',
          name: 'overrideContext',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'ShapePredictionStore',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.ShapePredictionStore.predictedContext',
          name: 'predictedContext',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'ShapePredictionStore',
          schema: 'public',
          nullable: false,
          default: "'GENERAL'::text",
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.ShapePredictionStore.engine',
          name: 'engine',
          type: 'PredictionsEngine',
          typeId: 'public.PredictionsEngine',
          table: 'ShapePredictionStore',
          schema: 'public',
          nullable: false,
          default: '\'FINETUNED_BERT\'::"PredictionsEngine"',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'E',
          constraints: [],
        },
      ],
      parents: [],
      children: [],
      primaryKeys: {
        tableId: 'public.ShapePredictionStore',
        schema: 'public',
        table: 'ShapePredictionStore',
        dirty: false,
        keys: [
          {
            name: 'id',
            type: 'text',
          },
        ],
      },
      constraints: [
        {
          tableId: 'public.ShapePredictionStore',
          schema: 'public',
          table: 'ShapePredictionStore',
          dirty: false,
          name: 'ShapePredictionStore_pkey',
          columns: ['id'],
        },
      ],
    },
    {
      id: 'public.Snapshot',
      name: 'Snapshot',
      schema: 'public',
      partitioned: false,
      rows: 3133,
      bytes: 4956160,
      columns: [
        {
          id: 'public.Snapshot.id',
          name: 'id',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Snapshot',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['p'],
        },
        {
          id: 'public.Snapshot.uniqueName',
          name: 'uniqueName',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Snapshot',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.Snapshot.createdAt',
          name: 'createdAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'Snapshot',
          schema: 'public',
          nullable: false,
          default: 'CURRENT_TIMESTAMP',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.Snapshot.updatedAt',
          name: 'updatedAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'Snapshot',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.Snapshot.statusOld',
          name: 'statusOld',
          type: 'SnapshotStatus',
          typeId: 'public.SnapshotStatus',
          table: 'Snapshot',
          schema: 'public',
          nullable: false,
          default: '\'BOOTING\'::"SnapshotStatus"',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'E',
          constraints: [],
        },
        {
          id: 'public.Snapshot.organizationId',
          name: 'organizationId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Snapshot',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
        {
          id: 'public.Snapshot.dbConnectionId',
          name: 'dbConnectionId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Snapshot',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
        {
          id: 'public.Snapshot.workerIpAddress',
          name: 'workerIpAddress',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Snapshot',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.Snapshot.errors',
          name: 'errors',
          type: 'text[]',
          typeId: 'pg_catalog._text',
          table: 'Snapshot',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'A',
          constraints: [],
        },
        {
          id: 'public.Snapshot.failureCount',
          name: 'failureCount',
          type: 'int4',
          typeId: 'pg_catalog.int4',
          table: 'Snapshot',
          schema: 'public',
          nullable: false,
          default: '0',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: [],
        },
        {
          id: 'public.Snapshot.projectId',
          name: 'projectId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Snapshot',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
        {
          id: 'public.Snapshot.dbSchemaDump',
          name: 'dbSchemaDump',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Snapshot',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.Snapshot.logs',
          name: 'logs',
          type: 'text[]',
          typeId: 'pg_catalog._text',
          table: 'Snapshot',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'A',
          constraints: [],
        },
        {
          id: 'public.Snapshot.restoreCount',
          name: 'restoreCount',
          type: 'int4',
          typeId: 'pg_catalog.int4',
          table: 'Snapshot',
          schema: 'public',
          nullable: false,
          default: '0',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: [],
        },
        {
          id: 'public.Snapshot.dbInfo',
          name: 'dbInfo',
          type: 'jsonb',
          typeId: 'pg_catalog.jsonb',
          table: 'Snapshot',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'U',
          constraints: [],
        },
        {
          id: 'public.Snapshot.snapshotConfig',
          name: 'snapshotConfig',
          type: 'jsonb',
          typeId: 'pg_catalog.jsonb',
          table: 'Snapshot',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'U',
          constraints: [],
        },
        {
          id: 'public.Snapshot.runtime',
          name: 'runtime',
          type: 'jsonb',
          typeId: 'pg_catalog.jsonb',
          table: 'Snapshot',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'U',
          constraints: [],
        },
        {
          id: 'public.Snapshot.summary',
          name: 'summary',
          type: 'jsonb',
          typeId: 'pg_catalog.jsonb',
          table: 'Snapshot',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'U',
          constraints: [],
        },
        {
          id: 'public.Snapshot.createdByUserId',
          name: 'createdByUserId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Snapshot',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
        {
          id: 'public.Snapshot.execTaskId',
          name: 'execTaskId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Snapshot',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
        {
          id: 'public.Snapshot.progress',
          name: 'progress',
          type: 'jsonb',
          typeId: 'pg_catalog.jsonb',
          table: 'Snapshot',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'U',
          constraints: [],
        },
        {
          id: 'public.Snapshot.notifyOnSuccess',
          name: 'notifyOnSuccess',
          type: 'bool',
          typeId: 'pg_catalog.bool',
          table: 'Snapshot',
          schema: 'public',
          nullable: true,
          default: 'false',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'B',
          constraints: [],
        },
        {
          id: 'public.Snapshot.deletedAt',
          name: 'deletedAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'Snapshot',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.Snapshot.purgedAt',
          name: 'purgedAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'Snapshot',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.Snapshot.storage',
          name: 'storage',
          type: 'jsonb',
          typeId: 'pg_catalog.jsonb',
          table: 'Snapshot',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'U',
          constraints: [],
        },
        {
          id: 'public.Snapshot.isScheduled',
          name: 'isScheduled',
          type: 'bool',
          typeId: 'pg_catalog.bool',
          table: 'Snapshot',
          schema: 'public',
          nullable: true,
          default: 'false',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'B',
          constraints: [],
        },
        {
          id: 'public.Snapshot.preseedPreviewDatabase',
          name: 'preseedPreviewDatabase',
          type: 'bool',
          typeId: 'pg_catalog.bool',
          table: 'Snapshot',
          schema: 'public',
          nullable: false,
          default: 'false',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'B',
          constraints: [],
        },
      ],
      parents: [
        {
          id: 'Snapshot_dbConnectionId_fkey',
          fkTable: 'public.Snapshot',
          keys: [
            {
              fkColumn: 'dbConnectionId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.DbConnection',
        },
        {
          id: 'Snapshot_execTaskId_fkey',
          fkTable: 'public.Snapshot',
          keys: [
            {
              fkColumn: 'execTaskId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.ExecTask',
        },
        {
          id: 'Snapshot_organizationId_fkey',
          fkTable: 'public.Snapshot',
          keys: [
            {
              fkColumn: 'organizationId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.Organization',
        },
        {
          id: 'Snapshot_projectId_fkey',
          fkTable: 'public.Snapshot',
          keys: [
            {
              fkColumn: 'projectId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.Project',
        },
        {
          id: 'Snapshot_createdByUserId_fkey',
          fkTable: 'public.Snapshot',
          keys: [
            {
              fkColumn: 'createdByUserId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.User',
        },
      ],
      children: [
        {
          id: 'AwsConsumptionHistory_snapshotId_fkey',
          fkTable: 'public.AwsConsumptionHistory',
          keys: [
            {
              fkColumn: 'snapshotId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.Snapshot',
        },
        {
          id: 'NeonConsumptionHistory_snapshotId_fkey',
          fkTable: 'public.NeonConsumptionHistory',
          keys: [
            {
              fkColumn: 'snapshotId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.Snapshot',
        },
        {
          id: 'NeonProject_snapshotId_fkey',
          fkTable: 'public.NeonProject',
          keys: [
            {
              fkColumn: 'snapshotId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.Snapshot',
        },
        {
          id: 'Table_snapshotId_fkey',
          fkTable: 'public.Table',
          keys: [
            {
              fkColumn: 'snapshotId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.Snapshot',
        },
      ],
      primaryKeys: {
        tableId: 'public.Snapshot',
        schema: 'public',
        table: 'Snapshot',
        dirty: false,
        keys: [
          {
            name: 'id',
            type: 'text',
          },
        ],
      },
      constraints: [
        {
          tableId: 'public.Snapshot',
          schema: 'public',
          table: 'Snapshot',
          dirty: false,
          name: 'Snapshot_pkey',
          columns: ['id'],
        },
      ],
    },
    {
      id: 'public.SupabaseProject',
      name: 'SupabaseProject',
      schema: 'public',
      partitioned: false,
      rows: 0,
      bytes: 0,
      columns: [
        {
          id: 'public.SupabaseProject.id',
          name: 'id',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'SupabaseProject',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['p'],
        },
        {
          id: 'public.SupabaseProject.createdAt',
          name: 'createdAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'SupabaseProject',
          schema: 'public',
          nullable: false,
          default: 'CURRENT_TIMESTAMP',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.SupabaseProject.updatedAt',
          name: 'updatedAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'SupabaseProject',
          schema: 'public',
          nullable: false,
          default: 'CURRENT_TIMESTAMP',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.SupabaseProject.projectId',
          name: 'projectId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'SupabaseProject',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.SupabaseProject.supabaseAuthCodeHash',
          name: 'supabaseAuthCodeHash',
          type: 'jsonb',
          typeId: 'pg_catalog.jsonb',
          table: 'SupabaseProject',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'U',
          constraints: [],
        },
        {
          id: 'public.SupabaseProject.supabaseRefreshToken',
          name: 'supabaseRefreshToken',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'SupabaseProject',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.SupabaseProject.supabaseAccessTokenHash',
          name: 'supabaseAccessTokenHash',
          type: 'jsonb',
          typeId: 'pg_catalog.jsonb',
          table: 'SupabaseProject',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'U',
          constraints: [],
        },
        {
          id: 'public.SupabaseProject.supabaseAccessTokenExpiresAt',
          name: 'supabaseAccessTokenExpiresAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'SupabaseProject',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
      ],
      parents: [],
      children: [
        {
          id: 'Project_supabaseProjectId_fkey',
          fkTable: 'public.Project',
          keys: [
            {
              fkColumn: 'supabaseProjectId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.SupabaseProject',
        },
      ],
      primaryKeys: {
        tableId: 'public.SupabaseProject',
        schema: 'public',
        table: 'SupabaseProject',
        dirty: false,
        keys: [
          {
            name: 'id',
            type: 'text',
          },
        ],
      },
      constraints: [
        {
          tableId: 'public.SupabaseProject',
          schema: 'public',
          table: 'SupabaseProject',
          dirty: false,
          name: 'SupabaseProject_pkey',
          columns: ['id'],
        },
      ],
    },
    {
      id: 'public.Table',
      name: 'Table',
      schema: 'public',
      partitioned: false,
      rows: 44868,
      bytes: 12189696,
      columns: [
        {
          id: 'public.Table.id',
          name: 'id',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Table',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['p'],
        },
        {
          id: 'public.Table.tableName',
          name: 'tableName',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Table',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.Table.status',
          name: 'status',
          type: 'SnapshotStatus',
          typeId: 'public.SnapshotStatus',
          table: 'Table',
          schema: 'public',
          nullable: false,
          default: '\'PENDING\'::"SnapshotStatus"',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'E',
          constraints: [],
        },
        {
          id: 'public.Table.bucketKey',
          name: 'bucketKey',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Table',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.Table.bytes',
          name: 'bytes',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Table',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.Table.timeToDump',
          name: 'timeToDump',
          type: 'int4',
          typeId: 'pg_catalog.int4',
          table: 'Table',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: [],
        },
        {
          id: 'public.Table.timeToSave',
          name: 'timeToSave',
          type: 'int4',
          typeId: 'pg_catalog.int4',
          table: 'Table',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: [],
        },
        {
          id: 'public.Table.snapshotId',
          name: 'snapshotId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Table',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
        {
          id: 'public.Table.organizationId',
          name: 'organizationId',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Table',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['f'],
        },
        {
          id: 'public.Table.checksum',
          name: 'checksum',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Table',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.Table.timeToCompress',
          name: 'timeToCompress',
          type: 'int4',
          typeId: 'pg_catalog.int4',
          table: 'Table',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: [],
        },
        {
          id: 'public.Table.timeToEncrypt',
          name: 'timeToEncrypt',
          type: 'int4',
          typeId: 'pg_catalog.int4',
          table: 'Table',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: [],
        },
        {
          id: 'public.Table.rows',
          name: 'rows',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Table',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.Table.schema',
          name: 'schema',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Table',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.Table.totalRows',
          name: 'totalRows',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'Table',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
      ],
      parents: [
        {
          id: 'Table_organizationId_fkey',
          fkTable: 'public.Table',
          keys: [
            {
              fkColumn: 'organizationId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.Organization',
        },
        {
          id: 'Table_snapshotId_fkey',
          fkTable: 'public.Table',
          keys: [
            {
              fkColumn: 'snapshotId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.Snapshot',
        },
      ],
      children: [],
      primaryKeys: {
        tableId: 'public.Table',
        schema: 'public',
        table: 'Table',
        dirty: false,
        keys: [
          {
            name: 'id',
            type: 'text',
          },
        ],
      },
      constraints: [
        {
          tableId: 'public.Table',
          schema: 'public',
          table: 'Table',
          dirty: false,
          name: 'Table_pkey',
          columns: ['id'],
        },
      ],
    },
    {
      id: 'public.User',
      name: 'User',
      schema: 'public',
      partitioned: false,
      rows: 15,
      bytes: 40960,
      columns: [
        {
          id: 'public.User.id',
          name: 'id',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'User',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['p'],
        },
        {
          id: 'public.User.sub',
          name: 'sub',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'User',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.User.email',
          name: 'email',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'User',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public.User.createdAt',
          name: 'createdAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'User',
          schema: 'public',
          nullable: false,
          default: 'CURRENT_TIMESTAMP',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.User.updatedAt',
          name: 'updatedAt',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'User',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public.User.role',
          name: 'role',
          type: 'UserRole',
          typeId: 'public.UserRole',
          table: 'User',
          schema: 'public',
          nullable: false,
          default: '\'USER\'::"UserRole"',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'E',
          constraints: [],
        },
        {
          id: 'public.User.notifications',
          name: 'notifications',
          type: 'UserNotifications',
          typeId: 'public.UserNotifications',
          table: 'User',
          schema: 'public',
          nullable: false,
          default: '\'EMAIL\'::"UserNotifications"',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'E',
          constraints: [],
        },
      ],
      parents: [],
      children: [
        {
          id: 'AccessToken_userId_fkey',
          fkTable: 'public.AccessToken',
          keys: [
            {
              fkColumn: 'userId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.User',
        },
        {
          id: 'AuditLog_userId_fkey',
          fkTable: 'public.AuditLog',
          keys: [
            {
              fkColumn: 'userId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.User',
        },
        {
          id: 'InviteToken_createdByUserId_fkey',
          fkTable: 'public.InviteToken',
          keys: [
            {
              fkColumn: 'createdByUserId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.User',
        },
        {
          id: 'Member_userId_fkey',
          fkTable: 'public.Member',
          keys: [
            {
              fkColumn: 'userId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: false,
            },
          ],
          targetTable: 'public.User',
        },
        {
          id: 'ReleaseVersion_userId_fkey',
          fkTable: 'public.ReleaseVersion',
          keys: [
            {
              fkColumn: 'userId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.User',
        },
        {
          id: 'Snapshot_createdByUserId_fkey',
          fkTable: 'public.Snapshot',
          keys: [
            {
              fkColumn: 'createdByUserId',
              fkType: 'text',
              targetColumn: 'id',
              targetType: 'text',
              nullable: true,
            },
          ],
          targetTable: 'public.User',
        },
      ],
      primaryKeys: {
        tableId: 'public.User',
        schema: 'public',
        table: 'User',
        dirty: false,
        keys: [
          {
            name: 'id',
            type: 'text',
          },
        ],
      },
      constraints: [
        {
          tableId: 'public.User',
          schema: 'public',
          table: 'User',
          dirty: false,
          name: 'User_pkey',
          columns: ['id'],
        },
      ],
    },
    {
      id: 'public._prisma_migrations',
      name: '_prisma_migrations',
      schema: 'public',
      partitioned: false,
      rows: 141,
      bytes: 65536,
      columns: [
        {
          id: 'public._prisma_migrations.id',
          name: 'id',
          type: 'varchar',
          typeId: 'pg_catalog.varchar',
          table: '_prisma_migrations',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: 36,
          identity: null,
          typeCategory: 'S',
          constraints: ['p'],
        },
        {
          id: 'public._prisma_migrations.checksum',
          name: 'checksum',
          type: 'varchar',
          typeId: 'pg_catalog.varchar',
          table: '_prisma_migrations',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: 64,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public._prisma_migrations.finished_at',
          name: 'finished_at',
          type: 'timestamptz',
          typeId: 'pg_catalog.timestamptz',
          table: '_prisma_migrations',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public._prisma_migrations.migration_name',
          name: 'migration_name',
          type: 'varchar',
          typeId: 'pg_catalog.varchar',
          table: '_prisma_migrations',
          schema: 'public',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: 255,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public._prisma_migrations.logs',
          name: 'logs',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: '_prisma_migrations',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'public._prisma_migrations.rolled_back_at',
          name: 'rolled_back_at',
          type: 'timestamptz',
          typeId: 'pg_catalog.timestamptz',
          table: '_prisma_migrations',
          schema: 'public',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public._prisma_migrations.started_at',
          name: 'started_at',
          type: 'timestamptz',
          typeId: 'pg_catalog.timestamptz',
          table: '_prisma_migrations',
          schema: 'public',
          nullable: false,
          default: 'now()',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'public._prisma_migrations.applied_steps_count',
          name: 'applied_steps_count',
          type: 'int4',
          typeId: 'pg_catalog.int4',
          table: '_prisma_migrations',
          schema: 'public',
          nullable: false,
          default: '0',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: [],
        },
      ],
      parents: [],
      children: [],
      primaryKeys: {
        tableId: 'public._prisma_migrations',
        schema: 'public',
        table: '_prisma_migrations',
        dirty: false,
        keys: [
          {
            name: 'id',
            type: 'varchar',
          },
        ],
      },
      constraints: [
        {
          tableId: 'public._prisma_migrations',
          schema: 'public',
          table: '_prisma_migrations',
          dirty: false,
          name: '_prisma_migrations_pkey',
          columns: ['id'],
        },
      ],
    },
    {
      id: 'pgboss.job',
      name: 'job',
      schema: 'pgboss',
      partitioned: false,
      rows: null,
      bytes: 0,
      columns: [
        {
          id: 'pgboss.job.id',
          name: 'id',
          type: 'uuid',
          typeId: 'pg_catalog.uuid',
          table: 'job',
          schema: 'pgboss',
          nullable: false,
          default: 'public.gen_random_uuid()',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'U',
          constraints: ['p'],
        },
        {
          id: 'pgboss.job.name',
          name: 'name',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'job',
          schema: 'pgboss',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'pgboss.job.priority',
          name: 'priority',
          type: 'int4',
          typeId: 'pg_catalog.int4',
          table: 'job',
          schema: 'pgboss',
          nullable: false,
          default: '0',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: [],
        },
        {
          id: 'pgboss.job.data',
          name: 'data',
          type: 'jsonb',
          typeId: 'pg_catalog.jsonb',
          table: 'job',
          schema: 'pgboss',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'U',
          constraints: [],
        },
        {
          id: 'pgboss.job.state',
          name: 'state',
          type: 'job_state',
          typeId: 'pgboss.job_state',
          table: 'job',
          schema: 'pgboss',
          nullable: false,
          default: "'created'::pgboss.job_state",
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'E',
          constraints: [],
        },
        {
          id: 'pgboss.job.retrylimit',
          name: 'retrylimit',
          type: 'int4',
          typeId: 'pg_catalog.int4',
          table: 'job',
          schema: 'pgboss',
          nullable: false,
          default: '0',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: [],
        },
        {
          id: 'pgboss.job.retrycount',
          name: 'retrycount',
          type: 'int4',
          typeId: 'pg_catalog.int4',
          table: 'job',
          schema: 'pgboss',
          nullable: false,
          default: '0',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: [],
        },
        {
          id: 'pgboss.job.retrydelay',
          name: 'retrydelay',
          type: 'int4',
          typeId: 'pg_catalog.int4',
          table: 'job',
          schema: 'pgboss',
          nullable: false,
          default: '0',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: [],
        },
        {
          id: 'pgboss.job.retrybackoff',
          name: 'retrybackoff',
          type: 'bool',
          typeId: 'pg_catalog.bool',
          table: 'job',
          schema: 'pgboss',
          nullable: false,
          default: 'false',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'B',
          constraints: [],
        },
        {
          id: 'pgboss.job.startafter',
          name: 'startafter',
          type: 'timestamptz',
          typeId: 'pg_catalog.timestamptz',
          table: 'job',
          schema: 'pgboss',
          nullable: false,
          default: 'now()',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'pgboss.job.startedon',
          name: 'startedon',
          type: 'timestamptz',
          typeId: 'pg_catalog.timestamptz',
          table: 'job',
          schema: 'pgboss',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'pgboss.job.singletonkey',
          name: 'singletonkey',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'job',
          schema: 'pgboss',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'pgboss.job.singletonon',
          name: 'singletonon',
          type: 'timestamp',
          typeId: 'pg_catalog.timestamp',
          table: 'job',
          schema: 'pgboss',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'pgboss.job.expirein',
          name: 'expirein',
          type: 'interval',
          typeId: 'pg_catalog.interval',
          table: 'job',
          schema: 'pgboss',
          nullable: false,
          default: "'00:15:00'::interval",
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'T',
          constraints: [],
        },
        {
          id: 'pgboss.job.createdon',
          name: 'createdon',
          type: 'timestamptz',
          typeId: 'pg_catalog.timestamptz',
          table: 'job',
          schema: 'pgboss',
          nullable: false,
          default: 'now()',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'pgboss.job.completedon',
          name: 'completedon',
          type: 'timestamptz',
          typeId: 'pg_catalog.timestamptz',
          table: 'job',
          schema: 'pgboss',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'pgboss.job.keepuntil',
          name: 'keepuntil',
          type: 'timestamptz',
          typeId: 'pg_catalog.timestamptz',
          table: 'job',
          schema: 'pgboss',
          nullable: false,
          default: "(now() + '14 days'::interval)",
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'pgboss.job.on_complete',
          name: 'on_complete',
          type: 'bool',
          typeId: 'pg_catalog.bool',
          table: 'job',
          schema: 'pgboss',
          nullable: false,
          default: 'false',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'B',
          constraints: [],
        },
        {
          id: 'pgboss.job.output',
          name: 'output',
          type: 'jsonb',
          typeId: 'pg_catalog.jsonb',
          table: 'job',
          schema: 'pgboss',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'U',
          constraints: [],
        },
      ],
      parents: [],
      children: [],
      primaryKeys: {
        tableId: 'pgboss.job',
        schema: 'pgboss',
        table: 'job',
        dirty: false,
        keys: [
          {
            name: 'id',
            type: 'uuid',
          },
        ],
      },
      constraints: [
        {
          tableId: 'pgboss.job',
          schema: 'pgboss',
          table: 'job',
          dirty: false,
          name: 'job_pkey',
          columns: ['id'],
        },
      ],
    },
    {
      id: 'pgboss.schedule',
      name: 'schedule',
      schema: 'pgboss',
      partitioned: false,
      rows: null,
      bytes: 0,
      columns: [
        {
          id: 'pgboss.schedule.name',
          name: 'name',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'schedule',
          schema: 'pgboss',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: ['p'],
        },
        {
          id: 'pgboss.schedule.cron',
          name: 'cron',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'schedule',
          schema: 'pgboss',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'pgboss.schedule.timezone',
          name: 'timezone',
          type: 'text',
          typeId: 'pg_catalog.text',
          table: 'schedule',
          schema: 'pgboss',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'S',
          constraints: [],
        },
        {
          id: 'pgboss.schedule.data',
          name: 'data',
          type: 'jsonb',
          typeId: 'pg_catalog.jsonb',
          table: 'schedule',
          schema: 'pgboss',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'U',
          constraints: [],
        },
        {
          id: 'pgboss.schedule.options',
          name: 'options',
          type: 'jsonb',
          typeId: 'pg_catalog.jsonb',
          table: 'schedule',
          schema: 'pgboss',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'U',
          constraints: [],
        },
        {
          id: 'pgboss.schedule.created_on',
          name: 'created_on',
          type: 'timestamptz',
          typeId: 'pg_catalog.timestamptz',
          table: 'schedule',
          schema: 'pgboss',
          nullable: false,
          default: 'now()',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'pgboss.schedule.updated_on',
          name: 'updated_on',
          type: 'timestamptz',
          typeId: 'pg_catalog.timestamptz',
          table: 'schedule',
          schema: 'pgboss',
          nullable: false,
          default: 'now()',
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
      ],
      parents: [],
      children: [],
      primaryKeys: {
        tableId: 'pgboss.schedule',
        schema: 'pgboss',
        table: 'schedule',
        dirty: false,
        keys: [
          {
            name: 'name',
            type: 'text',
          },
        ],
      },
      constraints: [
        {
          tableId: 'pgboss.schedule',
          schema: 'pgboss',
          table: 'schedule',
          dirty: false,
          name: 'schedule_pkey',
          columns: ['name'],
        },
      ],
    },
    {
      id: 'pgboss.version',
      name: 'version',
      schema: 'pgboss',
      partitioned: false,
      rows: null,
      bytes: 0,
      columns: [
        {
          id: 'pgboss.version.version',
          name: 'version',
          type: 'int4',
          typeId: 'pg_catalog.int4',
          table: 'version',
          schema: 'pgboss',
          nullable: false,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'N',
          constraints: ['p'],
        },
        {
          id: 'pgboss.version.maintained_on',
          name: 'maintained_on',
          type: 'timestamptz',
          typeId: 'pg_catalog.timestamptz',
          table: 'version',
          schema: 'pgboss',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
        {
          id: 'pgboss.version.cron_on',
          name: 'cron_on',
          type: 'timestamptz',
          typeId: 'pg_catalog.timestamptz',
          table: 'version',
          schema: 'pgboss',
          nullable: true,
          default: null,
          generated: 'NEVER',
          maxLength: null,
          identity: null,
          typeCategory: 'D',
          constraints: [],
        },
      ],
      parents: [],
      children: [],
      primaryKeys: {
        tableId: 'pgboss.version',
        schema: 'pgboss',
        table: 'version',
        dirty: false,
        keys: [
          {
            name: 'version',
            type: 'int4',
          },
        ],
      },
      constraints: [
        {
          tableId: 'pgboss.version',
          schema: 'pgboss',
          table: 'version',
          dirty: false,
          name: 'version_pkey',
          columns: ['version'],
        },
      ],
    },
  ],
  extensions: [
    {
      name: 'pgcrypto',
      version: '1.3',
      schema: 'public',
    },
  ],
  enums: [
    {
      schema: 'pgboss',
      name: 'job_state',
      id: 'pgboss.job_state',
      values: [
        'active',
        'cancelled',
        'completed',
        'created',
        'expired',
        'failed',
        'retry',
      ],
    },
    {
      schema: 'public',
      name: 'AccessTokenType',
      id: 'public.AccessTokenType',
      values: ['CLI', 'WORKER'],
    },
    {
      schema: 'public',
      name: 'AuditLogActions',
      id: 'public.AuditLogActions',
      values: [
        'EMAIL_SENT_INCOMPLETE_ONBOARDING',
        'EMAIL_SENT_ORGANIZATION_CHURN',
        'ORGANIZATION_DELETED',
        'PII_UPDATED_PREDICTIONS_OVERRIDE',
        'PREVIEW_DATABASE_CREATED',
        'PREVIEW_DATABASE_DEPLOYED',
        'PREVIEW_DATABASE_DESTROYED',
        'PREVIEW_DATABASE_DROPPED',
        'PREVIEW_DATABASE_RESET',
        'PROJECT_DELETED',
        'SNAPSHOT_CONFIG_UPDATED',
        'SNAPSHOT_CREATED',
        'SNAPSHOT_DELETED',
        'SNAPSHOT_RESTORED_FAILURE',
        'SNAPSHOT_RESTORED_SUCCESS',
      ],
    },
    {
      schema: 'public',
      name: 'DatabaseStatus',
      id: 'public.DatabaseStatus',
      values: ['DELETED', 'DISABLED', 'ENABLED'],
    },
    {
      schema: 'public',
      name: 'MemberRole',
      id: 'public.MemberRole',
      values: ['ADMIN', 'MEMBER', 'OWNER'],
    },
    {
      schema: 'public',
      name: 'PredictionsEngine',
      id: 'public.PredictionsEngine',
      values: ['FINETUNED_BERT'],
    },
    {
      schema: 'public',
      name: 'PreviewDatabaseStatus',
      id: 'public.PreviewDatabaseStatus',
      values: ['FAILURE', 'IN_PROGRESS', 'SUCCESS'],
    },
    {
      schema: 'public',
      name: 'ReleaseChannel',
      id: 'public.ReleaseChannel',
      values: ['BETA', 'PRIVATE', 'PUBLIC'],
    },
    {
      schema: 'public',
      name: 'SnapshotStatus',
      id: 'public.SnapshotStatus',
      values: [
        'BOOTING',
        'DELETED',
        'FAILURE',
        'IN_PROGRESS',
        'PENDING',
        'PURGED',
        'STARTED',
        'STARTING',
        'SUCCESS',
        'TIMEOUT',
      ],
    },
    {
      schema: 'public',
      name: 'UserNotifications',
      id: 'public.UserNotifications',
      values: ['EMAIL', 'NONE'],
    },
    {
      schema: 'public',
      name: 'UserRole',
      id: 'public.UserRole',
      values: ['ADMIN', 'SUPERUSER', 'USER'],
    },
  ],
  server: {
    version: '14.9 (Homebrew)',
  },
  indexes: [
    {
      schema: 'pgboss',
      table: 'job',
      index: 'job_fetch',
      definition:
        "CREATE INDEX job_fetch ON pgboss.job USING btree (name text_pattern_ops, startafter) WHERE (state < 'active'::pgboss.job_state)",
      type: 'btree',
      indexColumns: ['name', 'startafter'],
    },
    {
      schema: 'pgboss',
      table: 'job',
      index: 'job_name',
      definition:
        'CREATE INDEX job_name ON pgboss.job USING btree (name text_pattern_ops)',
      type: 'btree',
      indexColumns: ['name'],
    },
    {
      schema: 'pgboss',
      table: 'job',
      index: 'job_pkey',
      definition: 'CREATE UNIQUE INDEX job_pkey ON pgboss.job USING btree (id)',
      type: 'btree',
      indexColumns: ['id'],
    },
    {
      schema: 'pgboss',
      table: 'job',
      index: 'job_singleton_queue',
      definition:
        "CREATE UNIQUE INDEX job_singleton_queue ON pgboss.job USING btree (name, singletonkey) WHERE ((state < 'active'::pgboss.job_state) AND (singletonon IS NULL) AND (singletonkey = '__pgboss__singleton_queue'::text))",
      type: 'btree',
      indexColumns: ['name', 'singletonkey'],
    },
    {
      schema: 'pgboss',
      table: 'job',
      index: 'job_singletonkey',
      definition:
        "CREATE UNIQUE INDEX job_singletonkey ON pgboss.job USING btree (name, singletonkey) WHERE ((state < 'completed'::pgboss.job_state) AND (singletonon IS NULL) AND (NOT (singletonkey = '__pgboss__singleton_queue'::text)))",
      type: 'btree',
      indexColumns: ['name', 'singletonkey'],
    },
    {
      schema: 'pgboss',
      table: 'job',
      index: 'job_singletonkeyon',
      definition:
        "CREATE UNIQUE INDEX job_singletonkeyon ON pgboss.job USING btree (name, singletonon, singletonkey) WHERE (state < 'expired'::pgboss.job_state)",
      type: 'btree',
      indexColumns: ['name', 'singletonkey', 'singletonon'],
    },
    {
      schema: 'pgboss',
      table: 'job',
      index: 'job_singletonon',
      definition:
        "CREATE UNIQUE INDEX job_singletonon ON pgboss.job USING btree (name, singletonon) WHERE ((state < 'expired'::pgboss.job_state) AND (singletonkey IS NULL))",
      type: 'btree',
      indexColumns: ['name', 'singletonon'],
    },
    {
      schema: 'pgboss',
      table: 'schedule',
      index: 'schedule_pkey',
      definition:
        'CREATE UNIQUE INDEX schedule_pkey ON pgboss.schedule USING btree (name)',
      type: 'btree',
      indexColumns: ['name'],
    },
    {
      schema: 'pgboss',
      table: 'version',
      index: 'version_pkey',
      definition:
        'CREATE UNIQUE INDEX version_pkey ON pgboss.version USING btree (version)',
      type: 'btree',
      indexColumns: ['version'],
    },
    {
      schema: 'public',
      table: 'AccessToken',
      index: 'AccessToken_hash_key',
      definition:
        'CREATE UNIQUE INDEX "AccessToken_hash_key" ON public."AccessToken" USING btree (hash)',
      type: 'btree',
      indexColumns: ['hash'],
    },
    {
      schema: 'public',
      table: 'AccessToken',
      index: 'AccessToken_pkey',
      definition:
        'CREATE UNIQUE INDEX "AccessToken_pkey" ON public."AccessToken" USING btree (id)',
      type: 'btree',
      indexColumns: ['id'],
    },
    {
      schema: 'public',
      table: 'AccessToken',
      index: 'AccessToken_userId_idx',
      definition:
        'CREATE INDEX "AccessToken_userId_idx" ON public."AccessToken" USING btree ("userId")',
      type: 'btree',
      indexColumns: ['userId'],
    },
    {
      schema: 'public',
      table: 'AuditLog',
      index: 'AuditLog_action_idx',
      definition:
        'CREATE INDEX "AuditLog_action_idx" ON public."AuditLog" USING btree (action)',
      type: 'btree',
      indexColumns: ['action'],
    },
    {
      schema: 'public',
      table: 'AuditLog',
      index: 'AuditLog_organizationId_idx',
      definition:
        'CREATE INDEX "AuditLog_organizationId_idx" ON public."AuditLog" USING btree ("organizationId")',
      type: 'btree',
      indexColumns: ['organizationId'],
    },
    {
      schema: 'public',
      table: 'AuditLog',
      index: 'AuditLog_pkey',
      definition:
        'CREATE UNIQUE INDEX "AuditLog_pkey" ON public."AuditLog" USING btree (id)',
      type: 'btree',
      indexColumns: ['id'],
    },
    {
      schema: 'public',
      table: 'AwsConsumptionHistory',
      index: 'AwsConsumptionHistory_name_snapshotId_projectId_organizatio_key',
      definition:
        'CREATE UNIQUE INDEX "AwsConsumptionHistory_name_snapshotId_projectId_organizatio_key" ON public."AwsConsumptionHistory" USING btree (name, "snapshotId", "projectId", "organizationId", "startPeriod", "endPeriod")',
      type: 'btree',
      indexColumns: [
        'endPeriod',
        'name',
        'organizationId',
        'projectId',
        'snapshotId',
        'startPeriod',
      ],
    },
    {
      schema: 'public',
      table: 'AwsConsumptionHistory',
      index: 'AwsConsumptionHistory_organizationId_projectId_snapshotId_idx',
      definition:
        'CREATE INDEX "AwsConsumptionHistory_organizationId_projectId_snapshotId_idx" ON public."AwsConsumptionHistory" USING btree ("organizationId", "projectId", "snapshotId")',
      type: 'btree',
      indexColumns: ['organizationId', 'projectId', 'snapshotId'],
    },
    {
      schema: 'public',
      table: 'AwsConsumptionHistory',
      index: 'AwsConsumptionHistory_pkey',
      definition:
        'CREATE UNIQUE INDEX "AwsConsumptionHistory_pkey" ON public."AwsConsumptionHistory" USING btree (name, "organizationId", "startPeriod", "endPeriod")',
      type: 'btree',
      indexColumns: ['endPeriod', 'name', 'organizationId', 'startPeriod'],
    },
    {
      schema: 'public',
      table: 'AwsConsumptionHistory',
      index: 'AwsConsumptionHistory_startPeriod_endPeriod_idx',
      definition:
        'CREATE INDEX "AwsConsumptionHistory_startPeriod_endPeriod_idx" ON public."AwsConsumptionHistory" USING btree ("startPeriod", "endPeriod")',
      type: 'btree',
      indexColumns: ['endPeriod', 'startPeriod'],
    },
    {
      schema: 'public',
      table: 'DatabaseProvider',
      index: 'DatabaseProvider_domain_key',
      definition:
        'CREATE UNIQUE INDEX "DatabaseProvider_domain_key" ON public."DatabaseProvider" USING btree (domain)',
      type: 'btree',
      indexColumns: ['domain'],
    },
    {
      schema: 'public',
      table: 'DatabaseProvider',
      index: 'DatabaseProvider_pkey',
      definition:
        'CREATE UNIQUE INDEX "DatabaseProvider_pkey" ON public."DatabaseProvider" USING btree (id)',
      type: 'btree',
      indexColumns: ['id'],
    },
    {
      schema: 'public',
      table: 'DbConnection',
      index: 'DbConnection_id_organizationId_key',
      definition:
        'CREATE UNIQUE INDEX "DbConnection_id_organizationId_key" ON public."DbConnection" USING btree (id, "organizationId")',
      type: 'btree',
      indexColumns: ['id', 'organizationId'],
    },
    {
      schema: 'public',
      table: 'DbConnection',
      index: 'DbConnection_organizationId_idx',
      definition:
        'CREATE INDEX "DbConnection_organizationId_idx" ON public."DbConnection" USING btree ("organizationId")',
      type: 'btree',
      indexColumns: ['organizationId'],
    },
    {
      schema: 'public',
      table: 'DbConnection',
      index: 'DbConnection_pkey',
      definition:
        'CREATE UNIQUE INDEX "DbConnection_pkey" ON public."DbConnection" USING btree (id)',
      type: 'btree',
      indexColumns: ['id'],
    },
    {
      schema: 'public',
      table: 'ExecTask',
      index: 'ExecTask_pkey',
      definition:
        'CREATE UNIQUE INDEX "ExecTask_pkey" ON public."ExecTask" USING btree (id)',
      type: 'btree',
      indexColumns: ['id'],
    },
    {
      schema: 'public',
      table: 'InviteToken',
      index: 'InviteToken_pkey',
      definition:
        'CREATE UNIQUE INDEX "InviteToken_pkey" ON public."InviteToken" USING btree (token)',
      type: 'btree',
      indexColumns: ['token'],
    },
    {
      schema: 'public',
      table: 'InviteToken',
      index: 'InviteToken_usedByMemberId_key',
      definition:
        'CREATE UNIQUE INDEX "InviteToken_usedByMemberId_key" ON public."InviteToken" USING btree ("usedByMemberId")',
      type: 'btree',
      indexColumns: ['usedByMemberId'],
    },
    {
      schema: 'public',
      table: 'Member',
      index: 'Member_pkey',
      definition:
        'CREATE UNIQUE INDEX "Member_pkey" ON public."Member" USING btree (id)',
      type: 'btree',
      indexColumns: ['id'],
    },
    {
      schema: 'public',
      table: 'Member',
      index: 'Member_userId_organizationId_idx',
      definition:
        'CREATE INDEX "Member_userId_organizationId_idx" ON public."Member" USING btree ("userId", "organizationId")',
      type: 'btree',
      indexColumns: ['organizationId', 'userId'],
    },
    {
      schema: 'public',
      table: 'Member',
      index: 'Member_userId_organizationId_key',
      definition:
        'CREATE UNIQUE INDEX "Member_userId_organizationId_key" ON public."Member" USING btree ("userId", "organizationId")',
      type: 'btree',
      indexColumns: ['organizationId', 'userId'],
    },
    {
      schema: 'public',
      table: 'NeonConsumptionHistory',
      index: 'NeonConsumptionHistory_name_snapshotId_projectId_organizati_key',
      definition:
        'CREATE UNIQUE INDEX "NeonConsumptionHistory_name_snapshotId_projectId_organizati_key" ON public."NeonConsumptionHistory" USING btree (name, "snapshotId", "projectId", "organizationId", "startPeriod", "endPeriod")',
      type: 'btree',
      indexColumns: [
        'endPeriod',
        'name',
        'organizationId',
        'projectId',
        'snapshotId',
        'startPeriod',
      ],
    },
    {
      schema: 'public',
      table: 'NeonConsumptionHistory',
      index: 'NeonConsumptionHistory_organizationId_projectId_snapshotId_idx',
      definition:
        'CREATE INDEX "NeonConsumptionHistory_organizationId_projectId_snapshotId_idx" ON public."NeonConsumptionHistory" USING btree ("organizationId", "projectId", "snapshotId")',
      type: 'btree',
      indexColumns: ['organizationId', 'projectId', 'snapshotId'],
    },
    {
      schema: 'public',
      table: 'NeonConsumptionHistory',
      index: 'NeonConsumptionHistory_pkey',
      definition:
        'CREATE UNIQUE INDEX "NeonConsumptionHistory_pkey" ON public."NeonConsumptionHistory" USING btree (name, "organizationId", "startPeriod", "endPeriod")',
      type: 'btree',
      indexColumns: ['endPeriod', 'name', 'organizationId', 'startPeriod'],
    },
    {
      schema: 'public',
      table: 'NeonConsumptionHistory',
      index: 'NeonConsumptionHistory_startPeriod_endPeriod_idx',
      definition:
        'CREATE INDEX "NeonConsumptionHistory_startPeriod_endPeriod_idx" ON public."NeonConsumptionHistory" USING btree ("startPeriod", "endPeriod")',
      type: 'btree',
      indexColumns: ['endPeriod', 'startPeriod'],
    },
    {
      schema: 'public',
      table: 'NeonProject',
      index: 'NeonProject_neonProjectId_key',
      definition:
        'CREATE UNIQUE INDEX "NeonProject_neonProjectId_key" ON public."NeonProject" USING btree ("neonProjectId")',
      type: 'btree',
      indexColumns: ['neonProjectId'],
    },
    {
      schema: 'public',
      table: 'NeonProject',
      index: 'NeonProject_pkey',
      definition:
        'CREATE UNIQUE INDEX "NeonProject_pkey" ON public."NeonProject" USING btree (id)',
      type: 'btree',
      indexColumns: ['id'],
    },
    {
      schema: 'public',
      table: 'NeonProject',
      index: 'NeonProject_snapshotId_key',
      definition:
        'CREATE UNIQUE INDEX "NeonProject_snapshotId_key" ON public."NeonProject" USING btree ("snapshotId")',
      type: 'btree',
      indexColumns: ['snapshotId'],
    },
    {
      schema: 'public',
      table: 'Organization',
      index: 'Organization_pkey',
      definition:
        'CREATE UNIQUE INDEX "Organization_pkey" ON public."Organization" USING btree (id)',
      type: 'btree',
      indexColumns: ['id'],
    },
    {
      schema: 'public',
      table: 'PredictionDataSet',
      index: 'PredictionDataSet_input_context_idx',
      definition:
        'CREATE INDEX "PredictionDataSet_input_context_idx" ON public."PredictionDataSet" USING btree (input, context)',
      type: 'btree',
      indexColumns: ['context', 'input'],
    },
    {
      schema: 'public',
      table: 'PredictionDataSet',
      index: 'PredictionDataSet_input_key',
      definition:
        'CREATE UNIQUE INDEX "PredictionDataSet_input_key" ON public."PredictionDataSet" USING btree (input)',
      type: 'btree',
      indexColumns: ['input'],
    },
    {
      schema: 'public',
      table: 'PredictionDataSet',
      index: 'PredictionDataSet_input_shape_idx',
      definition:
        'CREATE INDEX "PredictionDataSet_input_shape_idx" ON public."PredictionDataSet" USING btree (input, shape)',
      type: 'btree',
      indexColumns: ['input', 'shape'],
    },
    {
      schema: 'public',
      table: 'PredictionDataSet',
      index: 'PredictionDataSet_pkey',
      definition:
        'CREATE UNIQUE INDEX "PredictionDataSet_pkey" ON public."PredictionDataSet" USING btree (id)',
      type: 'btree',
      indexColumns: ['id'],
    },
    {
      schema: 'public',
      table: 'PredictionJob',
      index: 'PredictionJob_pkey',
      definition:
        'CREATE UNIQUE INDEX "PredictionJob_pkey" ON public."PredictionJob" USING btree (id)',
      type: 'btree',
      indexColumns: ['id'],
    },
    {
      schema: 'public',
      table: 'PreviewDatabase',
      index: 'PreviewDatabase_neonProjectId_idx',
      definition:
        'CREATE INDEX "PreviewDatabase_neonProjectId_idx" ON public."PreviewDatabase" USING btree ("neonProjectId")',
      type: 'btree',
      indexColumns: ['neonProjectId'],
    },
    {
      schema: 'public',
      table: 'PreviewDatabase',
      index: 'PreviewDatabase_neonProjectId_name_key',
      definition:
        'CREATE UNIQUE INDEX "PreviewDatabase_neonProjectId_name_key" ON public."PreviewDatabase" USING btree ("neonProjectId", name)',
      type: 'btree',
      indexColumns: ['name', 'neonProjectId'],
    },
    {
      schema: 'public',
      table: 'PreviewDatabase',
      index: 'PreviewDatabase_neonProjectId_neonBranchId_key',
      definition:
        'CREATE UNIQUE INDEX "PreviewDatabase_neonProjectId_neonBranchId_key" ON public."PreviewDatabase" USING btree ("neonProjectId", "neonBranchId")',
      type: 'btree',
      indexColumns: ['neonBranchId', 'neonProjectId'],
    },
    {
      schema: 'public',
      table: 'PreviewDatabase',
      index: 'PreviewDatabase_pkey',
      definition:
        'CREATE UNIQUE INDEX "PreviewDatabase_pkey" ON public."PreviewDatabase" USING btree (id)',
      type: 'btree',
      indexColumns: ['id'],
    },
    {
      schema: 'public',
      table: 'PricingPlan',
      index: 'PricingPlan_pkey',
      definition:
        'CREATE UNIQUE INDEX "PricingPlan_pkey" ON public."PricingPlan" USING btree (id)',
      type: 'btree',
      indexColumns: ['id'],
    },
    {
      schema: 'public',
      table: 'PricingPlan',
      index: 'PricingPlan_productId_key',
      definition:
        'CREATE UNIQUE INDEX "PricingPlan_productId_key" ON public."PricingPlan" USING btree ("productId")',
      type: 'btree',
      indexColumns: ['productId'],
    },
    {
      schema: 'public',
      table: 'Project',
      index: 'Project_hostedDbUrlId_key',
      definition:
        'CREATE UNIQUE INDEX "Project_hostedDbUrlId_key" ON public."Project" USING btree ("hostedDbUrlId")',
      type: 'btree',
      indexColumns: ['hostedDbUrlId'],
    },
    {
      schema: 'public',
      table: 'Project',
      index: 'Project_organizationId_dbConnectionId_key',
      definition:
        'CREATE UNIQUE INDEX "Project_organizationId_dbConnectionId_key" ON public."Project" USING btree ("organizationId", "dbConnectionId")',
      type: 'btree',
      indexColumns: ['dbConnectionId', 'organizationId'],
    },
    {
      schema: 'public',
      table: 'Project',
      index: 'Project_organizationId_idx',
      definition:
        'CREATE INDEX "Project_organizationId_idx" ON public."Project" USING btree ("organizationId")',
      type: 'btree',
      indexColumns: ['organizationId'],
    },
    {
      schema: 'public',
      table: 'Project',
      index: 'Project_pkey',
      definition:
        'CREATE UNIQUE INDEX "Project_pkey" ON public."Project" USING btree (id)',
      type: 'btree',
      indexColumns: ['id'],
    },
    {
      schema: 'public',
      table: 'Project',
      index: 'Project_predictionJobId_key',
      definition:
        'CREATE UNIQUE INDEX "Project_predictionJobId_key" ON public."Project" USING btree ("predictionJobId")',
      type: 'btree',
      indexColumns: ['predictionJobId'],
    },
    {
      schema: 'public',
      table: 'Project',
      index: 'Project_supabaseProjectId_key',
      definition:
        'CREATE UNIQUE INDEX "Project_supabaseProjectId_key" ON public."Project" USING btree ("supabaseProjectId")',
      type: 'btree',
      indexColumns: ['supabaseProjectId'],
    },
    {
      schema: 'public',
      table: 'ReleaseVersion',
      index: 'ReleaseVersion_pkey',
      definition:
        'CREATE UNIQUE INDEX "ReleaseVersion_pkey" ON public."ReleaseVersion" USING btree (version)',
      type: 'btree',
      indexColumns: ['version'],
    },
    {
      schema: 'public',
      table: 'ReleaseVersion',
      index: 'ReleaseVersion_userId_key',
      definition:
        'CREATE UNIQUE INDEX "ReleaseVersion_userId_key" ON public."ReleaseVersion" USING btree ("userId")',
      type: 'btree',
      indexColumns: ['userId'],
    },
    {
      schema: 'public',
      table: 'ReleaseVersion',
      index: 'ReleaseVersion_version_userId_key',
      definition:
        'CREATE UNIQUE INDEX "ReleaseVersion_version_userId_key" ON public."ReleaseVersion" USING btree (version, "userId")',
      type: 'btree',
      indexColumns: ['userId', 'version'],
    },
    {
      schema: 'public',
      table: 'ShapePredictionOverride',
      index: 'ShapePredictionOverride_input_key',
      definition:
        'CREATE UNIQUE INDEX "ShapePredictionOverride_input_key" ON public."ShapePredictionOverride" USING btree (input)',
      type: 'btree',
      indexColumns: ['input'],
    },
    {
      schema: 'public',
      table: 'ShapePredictionOverride',
      index: 'ShapePredictionOverride_pkey',
      definition:
        'CREATE UNIQUE INDEX "ShapePredictionOverride_pkey" ON public."ShapePredictionOverride" USING btree (id)',
      type: 'btree',
      indexColumns: ['id'],
    },
    {
      schema: 'public',
      table: 'ShapePredictionOverride',
      index: 'ShapePredictionOverride_projectId_input_key',
      definition:
        'CREATE UNIQUE INDEX "ShapePredictionOverride_projectId_input_key" ON public."ShapePredictionOverride" USING btree ("projectId", input)',
      type: 'btree',
      indexColumns: ['input', 'projectId'],
    },
    {
      schema: 'public',
      table: 'ShapePredictionStore',
      index: 'ShapePredictionStore_input_engine_key',
      definition:
        'CREATE UNIQUE INDEX "ShapePredictionStore_input_engine_key" ON public."ShapePredictionStore" USING btree (input, engine)',
      type: 'btree',
      indexColumns: ['engine', 'input'],
    },
    {
      schema: 'public',
      table: 'ShapePredictionStore',
      index: 'ShapePredictionStore_input_predictedLabel_idx',
      definition:
        'CREATE INDEX "ShapePredictionStore_input_predictedLabel_idx" ON public."ShapePredictionStore" USING btree (input, "predictedLabel")',
      type: 'btree',
      indexColumns: ['input', 'predictedLabel'],
    },
    {
      schema: 'public',
      table: 'ShapePredictionStore',
      index: 'ShapePredictionStore_pkey',
      definition:
        'CREATE UNIQUE INDEX "ShapePredictionStore_pkey" ON public."ShapePredictionStore" USING btree (id)',
      type: 'btree',
      indexColumns: ['id'],
    },
    {
      schema: 'public',
      table: 'Snapshot',
      index: 'Snapshot_dbConnectionId_uniqueName_key',
      definition:
        'CREATE UNIQUE INDEX "Snapshot_dbConnectionId_uniqueName_key" ON public."Snapshot" USING btree ("dbConnectionId", "uniqueName")',
      type: 'btree',
      indexColumns: ['dbConnectionId', 'uniqueName'],
    },
    {
      schema: 'public',
      table: 'Snapshot',
      index: 'Snapshot_id_organizationId_key',
      definition:
        'CREATE UNIQUE INDEX "Snapshot_id_organizationId_key" ON public."Snapshot" USING btree (id, "organizationId")',
      type: 'btree',
      indexColumns: ['id', 'organizationId'],
    },
    {
      schema: 'public',
      table: 'Snapshot',
      index: 'Snapshot_pkey',
      definition:
        'CREATE UNIQUE INDEX "Snapshot_pkey" ON public."Snapshot" USING btree (id)',
      type: 'btree',
      indexColumns: ['id'],
    },
    {
      schema: 'public',
      table: 'Snapshot',
      index: 'Snapshot_uniqueName_key',
      definition:
        'CREATE UNIQUE INDEX "Snapshot_uniqueName_key" ON public."Snapshot" USING btree ("uniqueName")',
      type: 'btree',
      indexColumns: ['uniqueName'],
    },
    {
      schema: 'public',
      table: 'SupabaseProject',
      index: 'SupabaseProject_pkey',
      definition:
        'CREATE UNIQUE INDEX "SupabaseProject_pkey" ON public."SupabaseProject" USING btree (id)',
      type: 'btree',
      indexColumns: ['id'],
    },
    {
      schema: 'public',
      table: 'SupabaseProject',
      index: 'SupabaseProject_projectId_key',
      definition:
        'CREATE UNIQUE INDEX "SupabaseProject_projectId_key" ON public."SupabaseProject" USING btree ("projectId")',
      type: 'btree',
      indexColumns: ['projectId'],
    },
    {
      schema: 'public',
      table: 'Table',
      index: 'Table_id_organizationId_key',
      definition:
        'CREATE UNIQUE INDEX "Table_id_organizationId_key" ON public."Table" USING btree (id, "organizationId")',
      type: 'btree',
      indexColumns: ['id', 'organizationId'],
    },
    {
      schema: 'public',
      table: 'Table',
      index: 'Table_pkey',
      definition:
        'CREATE UNIQUE INDEX "Table_pkey" ON public."Table" USING btree (id)',
      type: 'btree',
      indexColumns: ['id'],
    },
    {
      schema: 'public',
      table: 'Table',
      index: 'Table_schema_tableName_snapshotId_key',
      definition:
        'CREATE UNIQUE INDEX "Table_schema_tableName_snapshotId_key" ON public."Table" USING btree (schema, "tableName", "snapshotId")',
      type: 'btree',
      indexColumns: ['schema', 'snapshotId', 'tableName'],
    },
    {
      schema: 'public',
      table: 'Table',
      index: 'Table_snapshotId_idx',
      definition:
        'CREATE INDEX "Table_snapshotId_idx" ON public."Table" USING btree ("snapshotId")',
      type: 'btree',
      indexColumns: ['snapshotId'],
    },
    {
      schema: 'public',
      table: 'User',
      index: 'User_email_key',
      definition:
        'CREATE UNIQUE INDEX "User_email_key" ON public."User" USING btree (email)',
      type: 'btree',
      indexColumns: ['email'],
    },
    {
      schema: 'public',
      table: 'User',
      index: 'User_pkey',
      definition:
        'CREATE UNIQUE INDEX "User_pkey" ON public."User" USING btree (id)',
      type: 'btree',
      indexColumns: ['id'],
    },
    {
      schema: 'public',
      table: 'User',
      index: 'User_sub_key',
      definition:
        'CREATE UNIQUE INDEX "User_sub_key" ON public."User" USING btree (sub)',
      type: 'btree',
      indexColumns: ['sub'],
    },
    {
      schema: 'public',
      table: '_prisma_migrations',
      index: '_prisma_migrations_pkey',
      definition:
        'CREATE UNIQUE INDEX _prisma_migrations_pkey ON public._prisma_migrations USING btree (id)',
      type: 'btree',
      indexColumns: ['id'],
    },
  ],
  sequences: {
    public: [
      {
        schema: 'public',
        name: 'DatabaseProvider_id_seq',
        start: 1,
        min: 1,
        max: 2147483647,
        current: 3,
        interval: 1,
      },
      {
        schema: 'public',
        name: 'Member_id_seq',
        start: 1,
        min: 1,
        max: 2147483647,
        current: 3290,
        interval: 1,
      },
      {
        schema: 'public',
        name: 'PricingPlan_id_seq',
        start: 1,
        min: 1,
        max: 2147483647,
        current: 1,
        interval: 1,
      },
    ],
  },
}
